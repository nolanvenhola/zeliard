...
```bash
# MASM 5.1 (canonical source and behavior oracle)
cd 3_Assembly/masm
...
# TASM 2.01 (historical compatibility build; not porting truth)
cd 3_Assembly/tasm
...
3_Assembly/
  tasm/                  Historical TASM 2.01 compatibility build
  ...
  masm/                  CANONICAL build — MASM 5.1 and behavior oracles
    working/             Canonical source (same 60 files)
    bin/                 Bit-perfect release output
...
`masm/working/drivers/stdply.inc`. TCRF authoritative names apply.
...
| Task | Edit | Why |
|---|---|---|
| RE work and symbol renames | `masm/working/` | Canonical source and release bytes |
| Web port reference | `masm/working/` | Only behavioral source of truth |
| Debug output and behavior probes | `masm/working/`, `masm/functest/` | Debug/oracle infrastructure |
| Historical compatibility checks | `tasm/working/` | Read-only unless explicitly requested |

MASM release/debug source and generated release bytes are the sole behavioral
authority for the C port. Do not infer behavior from the TASM tree or mirror
new implementation work into it unless explicitly requested.
...
- **Web port**: opening demo finalized; exact game bootstrap landed; oracle-backed town rendering in progress
...
| What | Where |
|---|---|
| Player record layout | `masm/working/drivers/stdply.inc` |
| All gvar_* globals | `masm/working/core/zeliard.inc` |
| Cavern engine (fight/combat) | `masm/working/zelres2/code/200FIGHT.asm` |
| Town engine | `masm/working/zelres1/code/106TOWN.asm` |
