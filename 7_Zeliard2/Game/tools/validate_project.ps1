...
Invoke-CheckedProcess -Executable "python" -Description "Check architecture and typed coding rules" -Arguments @(
    (Join-Path $PSScriptRoot "check_architecture.py")
)
...
if (-not $SkipExports) {
    ...
    $windowsExecutable = Join-Path $windowsOutput "Zeliard2.exe"
...
Write-Host "PASS: Zeliard 2 production project validation"
