...
Complete reverse engineering of Zeliard (Game Arts, 1987/1990) — a DOS action-RPG. The primary goal is a fully reconstructed, bit-perfect assembly source tree that compiles back to the original game binaries.
...
- **Full virtual filesystem** mapped — every chunk identified by original filename
...
- `zeliad.exe` — main loader
- `game.bin`, `gmmcga.bin`, `gmcga.bin`, etc. — driver/engine binaries
- `zelres1.sar`, `zelres2.sar`, `zelres3.sar` — resource archives (290 total chunks)
...
working/
  ...
  zelres1/code/   14 ASM files (100OPSCN through 130UTILB)
  zelres1/data/   28 data files (.grp, .msd)
  zelres2/code/   20 ASM files (200MGAME through 250GMENG)
...
- `CHUNKS/` — chunk walkthroughs, format docs, complete chunk index
- `mdt_dungeon_format.md` — dungeon map file format (.MDT)
- Driver and core module walkthroughs

### 4_Resources/
Reference materials — game manual (PDF), maps (BMP), MIDI music, sprites, playthrough notes.
...
| 50 | Gameplay engine module 2 (250GMENG) |
...
This repository contains only assembly reconstructions and tooling — no original game assets.
