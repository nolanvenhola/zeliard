...
Mechanics walkthroughs and the master investigation tracker currently live in
`3_Assembly/tasm/Documentation/` for historical reasons. The MASM release/debug
source and generated bytes—not the TASM tree—are authoritative when resolving
behavior or implementing the web port.
