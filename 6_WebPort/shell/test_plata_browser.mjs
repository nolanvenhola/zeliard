...
try {
  ...
  if (capturePrefix) {
    await page.locator('#plata-capture').screenshot({
      path: `${capturePrefix}-plata.png`,
    });
  }

  if (!result.started || !result.active || result.width !== 256 ||
      result.chunk !== 91 || result.music !== 14 || !result.exactAudio ||
      result.visible < 1000)
    throw new Error(`Plata browser parity failed: ${JSON.stringify(result)}`);
  if (errors.length)
    throw new Error(`browser errors: ${JSON.stringify(errors)}`);
  console.log(`plata_browser: PASS width=${result.width} ` +
    `music=${result.chunk}/${result.music} visible=${result.visible}`);
  console.log('VERDICT: PASS: Plata runtime and exact Area-6 audio');
} finally {
  await browser.close();
}
