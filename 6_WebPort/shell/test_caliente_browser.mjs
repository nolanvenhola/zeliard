...
try {
  ...
  if (capturePrefix) {
    await page.locator('#caliente-capture').screenshot({
      path: `${capturePrefix}-caliente.png`,
    });
  }
  if (!result.started || !result.active || result.width !== 208 ||
      result.chunk !== 92 || result.music !== 3 || !result.exactAudio ||
      result.visible < 1000)
    throw new Error(`Caliente browser parity failed: ${JSON.stringify(result)}`);
  if (errors.length)
    throw new Error(`browser errors: ${JSON.stringify(errors)}`);
  console.log(`caliente_browser: PASS width=${result.width} ` +
    `music=${result.chunk}/${result.music} visible=${result.visible}`);
  console.log('VERDICT: PASS: Caliente runtime and exact Area-7 audio');
} finally {
  await browser.close();
}
