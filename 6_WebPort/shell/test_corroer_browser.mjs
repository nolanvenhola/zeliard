...
const url = process.argv[2] ?? 'http://127.0.0.1:5192/';
...
try {
  ...
  const result = await page.evaluate(() => {
    ...
    // Present this exact VM frame before the page's normal opening scheduler
    // gets another animation callback.  This makes the optional capture a
    // faithful Corroer inspection artifact rather than a timing-dependent
    // opening-screen capture.
    const canvas = document.createElement('canvas');
    canvas.id = 'corroer-capture';
    canvas.width = 320;
    canvas.height = 200;
  ...
  if (capturePrefix) {
    await page.locator('#corroer-capture').screenshot({
      path: `${capturePrefix}-corroer.png`,
    });
  }

  if (!result.started || !result.active || result.width !== 240 ||
      result.chunk !== 90 || result.music !== 13 || !result.exactAudio ||
      result.visible < 1000)
    throw new Error(`Corroer browser parity failed: ${JSON.stringify(result)}`);
  if (errors.length)
    throw new Error(`browser errors: ${JSON.stringify(errors)}`);
  console.log(`corroer_browser: PASS width=${result.width} ` +
    `music=${result.chunk}/${result.music} visible=${result.visible}`);
  console.log('VERDICT: PASS: Corroer runtime and exact Area-5 audio');
} finally {
  await browser.close();
}
