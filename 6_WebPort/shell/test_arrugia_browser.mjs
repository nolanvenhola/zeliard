...
const url = process.argv[2] ?? 'http://127.0.0.1:5195/';
...
try {
  ...
  if (capturePrefix) {
    await page.locator('#arrugia-capture').screenshot({
      path: `${capturePrefix}-arrugia.png`,
    });
  }

  if (!result.started || !result.active || result.width !== 73 ||
      result.chunk !== 91 || result.music !== 14 || !result.exactAudio ||
      result.visible < 1000)
    throw new Error(`Arrugia browser parity failed: ${JSON.stringify(result)}`);
  if (errors.length)
    throw new Error(`browser errors: ${JSON.stringify(errors)}`);
  console.log(`arrugia_browser: PASS width=${result.width} ` +
    `music=${result.chunk}/${result.music} visible=${result.visible}`);
  console.log('VERDICT: PASS: Arrugia runtime and exact Area-6 audio');
} finally {
  await browser.close();
}
