...
try {
    ...
    $cmdLines = @(
        "@echo off",
        "call `"$vcvars`" >nul",
        "cl /nologo /std:c11 /W3 /O2 /DZELIARD_NO_MAIN /I. /Fobuild\obj\ /Fe:build\opening_parity_native.exe tests\opening_parity_native.c main.c core\framebuf.c core\runtime.c core\timer.c render\palette.c render\font_text.c load\grp.c load\fill_buffer.c load\img_open.c game\opening_script.c game\opening.c game\gameplay_state.c platform\platform_sdl.c",
        "cl /nologo /std:c11 /W3 /O2 /DZELIARD_NO_MAIN /I. /Fobuild\obj\ /Fe:build\opening_service_trace_native.exe tests\opening_service_trace_native.c main.c core\framebuf.c core\runtime.c core\timer.c render\palette.c render\font_text.c load\grp.c load\fill_buffer.c load\img_open.c game\opening_script.c game\opening.c game\gameplay_state.c platform\platform_sdl.c"
    )
    if (!$OpeningOnly) {
        $cmdLines += @(
            ...
            "cl /nologo /std:c11 /W3 /O2 /I. /Fobuild\obj\ /Fe:build\runtime_parity_native.exe tests\runtime_parity_native.c core\runtime.c core\framebuf.c core\timer.c render\palette.c platform\platform_sdl.c"
        )
    }
