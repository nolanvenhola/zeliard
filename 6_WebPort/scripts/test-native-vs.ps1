...
try {
    ...
    $cmdLines = @(
        "@echo off",
        "call `"$vcvars`" >nul",
        "cl /nologo /std:c11 /W3 /O2 /DZELIARD_NO_MAIN /I. /Fobuild\obj\ /Fe:build\opening_parity_native.exe tests\opening_parity_native.c main.c core\framebuf.c render\palette.c render\font_text.c load\grp.c load\fill_buffer.c load\img_open.c game\opening_script.c game\opening.c game\gameplay_state.c platform\platform_sdl.c",
