...
void zel_tiny86_reset(const unsigned char *bios, unsigned bios_size)
{
    ...
    /* 8086tiny's decoder tables are stored in its MIT-licensed BIOS image. */
...
int zel_tiny86_run(unsigned max_instructions)
{
           ...
           opcode_stream != mem && inst_counter - first_instruction < max_instructions;)
	{
		...
		switch (xlat_opcode_id)
		{
			...
			OPCODE 22: // OUT DX/imm8, AL/AX
				...
				if (out_callback)
					out_callback(out_callback_context, (unsigned short)scratch_uint,
					             regs8[REG_AL]);
	...
	return opcode_stream == mem ? ZEL_TINY86_HALTED : ZEL_TINY86_YIELDED;
}

unsigned char *zel_tiny86_memory(void) { return mem; }
unsigned zel_tiny86_memory_size(void) { return RAM_SIZE; }
unsigned short *zel_tiny86_registers(void) { return regs16; }
