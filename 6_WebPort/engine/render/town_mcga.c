...
static int draw_equipped_small(u8 *vga, size_t vga_size,
                               const u8 *item_seg, size_t item_size,
                               u8 item, u16 bx, u16 pointer_offset) {
    if (!vga || vga_size < 0x10000 || !item_seg ||
        item_size < (size_t)pointer_offset + 2u || item == 0)
        return -1;

    size_t source = read_u16_le(item_seg + pointer_offset) +
                    (size_t)(item - 1u) * 0xC0u;
...
int zeliard_gmmcga_draw_equipped_spell(u8 *vga, size_t vga_size,
                                       const u8 *item_seg, size_t item_size,
                                       u8 spell, u16 bx) {
    /* GMMCGA:25E2h selects the one-based spell frame through the relocated
     * itemp.grp pointer at game_seg:E206h. */
    return draw_equipped_small(vga, vga_size, item_seg, item_size,
                               spell, bx, 0xE206);
}
...
int zeliard_gmmcga_draw_equipped_shield(u8 *vga, size_t vga_size,
                                        const u8 *item_seg, size_t item_size,
                                        u8 shield, u16 bx) {
    /* GMMCGA:25FCh selects the one-based shield frame through the relocated
     * itemp.grp pointer at game_seg:E202h. */
    return draw_equipped_small(vga, vga_size, item_seg, item_size,
                               shield, bx, 0xE202);
}
...
int zeliard_gmmcga_draw_life_max(u8 *vga, size_t vga_size,
    ...
    const u16 width = life_width(
        zeliard_player_read_u16(&player, ZEL_PLAYER_HP_MAX));
...
int zeliard_gmmcga_draw_life_current(u8 *vga, size_t vga_size,
    ...
    zeliard_player_state_t player = {.bytes = (u8 *)game_seg};
    const u16 width = life_width(
        zeliard_player_read_u16(&player, ZEL_PLAYER_HP));
...
int zeliard_gmmcga_draw_almas(u8 *vga, size_t vga_size,
    ...
    return draw_hud_number(vga, vga_size, game_seg, game_size,
                           zeliard_player_read_u16(&player, ZEL_PLAYER_ALMAS), 0x2435,
                           0x26, 0xBB, 5, 1);
}
...
int zeliard_gmmcga_draw_gold(u8 *vga, size_t vga_size,
    ...
    zeliard_player_state_t player = {.bytes = game_seg};
    const u32 value = zeliard_player_read_u24(&player, ZEL_PLAYER_GOLD);
...
int zeliard_gmmcga_draw_spell_charge(u8 *vga, size_t vga_size,
    ...
    zeliard_player_state_t player = {.bytes = game_seg};
    const u8 selected =
        zeliard_player_read_u8(&player, ZEL_PLAYER_SELECTED_SPELL);
    const u16 charge_address =
        (u16)(ZEL_PLAYER_SPELL_CHARGES + (u8)(selected - 1));
    const u8 charge = charge_address < ZEL_PLAYER_RECORD_SIZE
        ? zeliard_player_read_u8(&player, charge_address)
        : game_seg[charge_address];
    return draw_hud_number(vga, vga_size, game_seg, game_size,
                           charge, 0x2437,
                           0x37, 0xBB, 3, 1);
}
...
int zeliard_gmmcga_draw_shield_hp(u8 *vga, size_t vga_size,
    ...
    zeliard_player_state_t player = {.bytes = game_seg};
    if (zeliard_player_read_u8(&player, ZEL_PLAYER_SHIELD) == 0) return 0;
    return draw_hud_number(vga, vga_size, game_seg, game_size,
                           zeliard_player_read_u16(&player, ZEL_PLAYER_SHIELD_HP),
                           0x2437,
                           0x3E, 0xBB, 3, 1);
}

/* GMMCGA:2A1C, resident dispatch slot CS:203E.  The release GAME code
 * renders the collected Tears of Esmesanti immediately after GFMCGA builds
 * the ornamental frame.  Tears 1..8 use the crystal sprite; the ninth uses
 * the completed-crown sprite.  0x80 is transparent. */
static const u8 TEAR_SPRITES[2][13][16] = {
    {
        {0x80,0x80,0x80,0x80,0x80,0x80,0x00,0x00,0x00,0x00,0x80,0x00,0x80,0x80,0x80,0x80},
        {0x80,0x80,0x80,0x80,0x00,0x00,0x11,0x11,0x11,0x12,0x00,0x00,0x80,0x80,0x80,0x80},
        {0x80,0x80,0x80,0x80,0x00,0x11,0x11,0x09,0x09,0x01,0x12,0x00,0x80,0x80,0x80,0x80},
        {0x80,0x80,0x80,0x80,0x00,0x11,0x09,0x09,0x09,0x28,0x2A,0x10,0x80,0x80,0x80,0x80},
        {0x80,0x80,0x80,0x80,0x11,0x15,0x01,0x09,0x0D,0x05,0x05,0x12,0x00,0x80,0x80,0x80},
        {0x80,0x80,0x80,0x80,0x11,0x10,0x28,0x28,0x2D,0x28,0x28,0x12,0x00,0x80,0x80,0x80},
        {0x80,0x80,0x80,0x80,0x12,0x15,0x05,0x05,0x05,0x05,0x05,0x12,0x00,0x80,0x80,0x80},
        {0x80,0x80,0x80,0x80,0x00,0x12,0x05,0x2D,0x2D,0x05,0x15,0x02,0x80,0x80,0x80,0x80},
        {0x80,0x80,0x80,0x80,0x00,0x02,0x02,0x2D,0x2D,0x05,0x12,0x00,0x80,0x80,0x80,0x80},
        {0x80,0x80,0x80,0x80,0x00,0x00,0x02,0x12,0x12,0x12,0x00,0x00,0x80,0x80,0x80,0x80},
        {0x80,0x80,0x80,0x80,0x00,0x00,0x80,0x00,0x00,0x00,0x80,0x00,0x80,0x80,0x80,0x80},
        {0x80,0x80,0x80,0x80,0x80,0x80,0x80,0x80,0x80,0x80,0x80,0x80,0x80,0x80,0x80,0x80},
        {0x80,0x80,0x80,0x80,0x80,0x80,0x80,0x80,0x80,0x80,0x80,0x80,0x80,0x80,0x80,0x80},
    },
    {
        {0x80,0x80,0x80,0x80,0x00,0x01,0x09,0x09,0x09,0x1B,0x03,0x00,0x80,0x80,0x80,0x80},
        {0x80,0x80,0x80,0x00,0x09,0x09,0x00,0x00,0x00,0x00,0x03,0x1B,0x00,0x80,0x80,0x80},
        {0x80,0x80,0x00,0x09,0x01,0x00,0x01,0x09,0x01,0x00,0x00,0x03,0x03,0x00,0x80,0x80},
        {0x80,0x80,0x01,0x09,0x00,0x09,0x09,0x01,0x00,0x00,0x01,0x00,0x03,0x03,0x80,0x80},
        {0x80,0x00,0x09,0x01,0x01,0x09,0x09,0x00,0x00,0x00,0x00,0x01,0x03,0x03,0x00,0x80},
        {0x80,0x00,0x09,0x00,0x09,0x01,0x00,0x00,0x02,0x02,0x00,0x00,0x00,0x0B,0x00,0x80},
        {0x80,0x00,0x09,0x00,0x01,0x00,0x00,0x02,0x02,0x02,0x02,0x02,0x02,0x0B,0x00,0x80},
        {0x80,0x00,0x09,0x03,0x01,0x02,0x02,0x02,0x12,0x12,0x12,0x02,0x01,0x0B,0x00,0x80},
        {0x80,0x80,0x01,0x1B,0x02,0x01,0x02,0x12,0x12,0x12,0x12,0x02,0x09,0x01,0x80,0x80},
        {0x80,0x80,0x00,0x0B,0x03,0x02,0x0A,0x01,0x12,0x12,0x12,0x01,0x09,0x00,0x80,0x80},
        {0x80,0x80,0x80,0x00,0x1B,0x03,0x02,0x00,0x02,0x02,0x01,0x09,0x00,0x80,0x80,0x80},
        {0x80,0x80,0x80,0x80,0x00,0x03,0x01,0x03,0x03,0x01,0x03,0x00,0x80,0x80,0x80,0x80},
...
int zeliard_gmmcga_draw_collected_tears(u8 *vga, size_t vga_size,
                                         const u8 *game_seg,
                                         size_t game_size) {
    static const u8 x_quads[9] = {0x0F, 0x3D, 0x15, 0x37, 0x1B,
                                  0x31, 0x21, 0x2B, 0x26};
    if (!vga || vga_size < 0x10000 || !game_seg ||
        game_size <= ZEL_PLAYER_TEARS)
        return -1;
    u8 count = game_seg[ZEL_PLAYER_TEARS];
    if (count > 9) count = 9;
    for (u8 tear = 0; tear < count; ++tear) {
        const u8 (*sprite)[16] = TEAR_SPRITES[tear == 8];
        const u16 x = (u16)x_quads[tear] * 4u;
        for (u8 y = 0; y < 13; ++y) {
            for (u8 sx = 0; sx < 16; ++sx) {
                const u8 color = sprite[y][sx];
                if (color != 0x80) vga[(u16)y * 320u + x + sx] = color;
            }
        }
    }
    return 0;
...
int zeliard_gmmcga_draw_first_frame_hud(u8 *vga, size_t vga_size,
    ...
    if (!vga || vga_size < 0x10000 || !game_seg || game_size < 0x10000)
        return -1;
    if (zeliard_gmmcga_draw_collected_tears(vga, vga_size, game_seg,
                                             game_size) ||
        zeliard_gmmcga_draw_status_line(vga, vga_size, 0, 0x0204, 0x2100) ||
    ...
    if (zeliard_player_read_u8(&player, ZEL_PLAYER_SELECTED_SPELL) != 0 &&
        (zeliard_gmmcga_draw_status_line(vga, vga_size, 0,
                                         0xAA1C, 0x1700) ||
         zeliard_gmmcga_draw_spell_charge(vga, vga_size,
                                           game_seg, game_size)))
        return -1;
...
int zeliard_gtmcga_render_town_actors(u8 *game_seg, size_t game_size,
    ...
    u16 position = (u16)(zeliard_player_read_u16(
        &player, ZEL_PLAYER_START_POSITION) + player_col + 4);
    ...
    const u16 npc_column = (u16)(zeliard_player_read_u16(
        &player, ZEL_PLAYER_START_POSITION) + player_col + 3);
    ...
    const u16 walk =
        (zeliard_player_read_u8(&player, ZEL_PLAYER_FACING_DIRECTION) & 1)
        ? 0x6A3B : 0x6A59;
    const u16 player_ids = (u16)(walk + (u16)zeliard_player_read_u8(
        &player, ZEL_PLAYER_POSE) * 6u);
...
static void update_actor_columns(u8 *game_seg, const u8 *game_data,
    ...
    zeliard_player_state_t player = {.bytes = game_seg};
    const u16 world = (u16)(zeliard_player_read_u16(
        &player, ZEL_PLAYER_START_POSITION) + column + 4u);
    u8 ids[6];
