...
static int map_shape(unsigned *monsters, unsigned *items,
                     unsigned *families) {
    size_t size = 0;
    u8 *image = platform_load_asset("mp50.mdt", &size);
...
int main(void) {
    ...
    printf("corroer_probe: started=%d active=%d frame=%d width=%u level=%u "
    ...
    ok &= monsters == 49 && items == 24 && families == 0x1E;

    const u16 objects = (u16)zeliard_fight_masm_vm_peek_u16(0xC010);
    u8 before[73][16];
    for (unsigned object = 0; object < 73; ++object)
    ...
    printf("corroer_ai_probe: objects=%04x changed=%02x frame=%016llx\n",
           objects, changed_families, moving_frame);
    ok &= changed_families == 0x1E;

    /* MP50 object one is backed by stdply byte 22h/mask 80h.  Both ordinary
     * pickups and sword-opened wall stashes use this authored persistence
     * path, so re-entry must remove the object record. */
    static u8 persistent_game[0x10000], persistent_vga[0x10000];
    prepare_player(persistent_game, 11, 88, 34);
    persistent_game[0x22] = 0x80;
    palette_set_game_mcga();
    ...
    for (unsigned byte = 0; byte < 16; ++byte)
        persisted[byte] = (u8)zeliard_fight_masm_vm_peek_u8(
            (u16)(persistent_objects + 16u + byte));
    printf("corroer_persistence_probe: objects=%04x head=%02x%02x "
    ...
    /* MP50 and MP51 x88/y34 are the exact bidirectional Corroer/Cementar
     * transition pair.  Exercise both directions without reconstructing the
     * map or player state in host code. */
    static u8 route_game[0x10000], route_vga[0x10000];
    prepare_player(route_game, 11, 88, 34);
    ...
    printf("corroer_route_probe: entered=%d/%04x returned=%d width=%u "
    ...
    ok &= zeliard_fight_masm_vm_exit_selector() == 0x85;

    /* These hashes pin the exact first and ten-frame Area-5 VM render once
     * generated from the canonical release assets. */
    ok &= first_frame == 0xC97D869A2A0B2576ULL;
    ok &= moving_frame == 0x61139C993884F616ULL;

    printf("VERDICT: %s: Corroer exact fight VM, persistence, and routes\n",
           ok ? "PASS" : "FAIL");
    return ok ? 0 : 1;
}
