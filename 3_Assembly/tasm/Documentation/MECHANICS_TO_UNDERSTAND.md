...
| Sound-effects on/off toggle (F2 key) | ✓ FAQ-CORRECTED | Per FAQ §11: F2 = SFX toggle, F1 = music toggle.  My earlier "F2 = music+SFX combined" was wrong.  F2 (bit 0x2000) toggles `gvar_sound_flag` (stick.asm:292) which gates SFX only.  F1 (bit 0x1000) calls INT 60h AX=2 = mscmt.drv music-toggle service (separate state).  See §14. |
...
| Gelroid (blue HP-drain fluid, Tumba) | ✓ FAQ-CONFIRMED | Per FAQ §8.1: "Gelroid is a blue fluid found in the Rotten Caverns [Tumba].  It sucks the life energy of anyone stepping into it and, the longer you stand, the more you get hurt.  Pirika Shoes required to cross (path too long otherwise).  Red Slimes inhabit the Gelroid".  Mechanism: standard `tile_type_sum` per-frame damage chain (TILE_PHYSICS.md), tile bytes at specific values in Tumba's `tile_type_map[16]`.  Pirika Shoes likely speed up traversal via a separate per-area gate proc (similar to Ruzeria/Cape pattern) — exact gate not yet pinned. |
...
| Sword swing direction variants (FAQ §11) | ✓ | Per FAQ keystroke help: **Up+Space** = swing sword all the way around (overhead arc), **Down+Space while falling** = stab attack, **Up+Down+Space** = stand still and swing (no jump), **Space alone with enemy above** = swing without jumping.  All variants map to the same `combat_input_handler` FSM (`gvar_combat_action_state=2`) — the directional modifier is applied via the joystick AL bits read at line 2624.  Combined with `select_player_sprite_frame`'s entity_ptr_loop auto-aim, the swing direction modifies which cells get hit-tested first. |
| Sword swing as defense (resting at safe spot) | ✓ | Per FAQ §10: "If you stand still, you will regain your strength.  You can also defend yourself by attacking with your sword while standing ground and not bending or jumping."  Mechanism: `combat_input_handler` with state==2 (ATTACK) places hit-test cells around the player (entity_ptr_loop); any incoming enemy entity gets the 0x41 hit marker before contacting player_HP.  HP regen at safe spots: `heal_pulse_count` decrements + auto-regen at 200FIGHT.asm:2941-2952 when not in active combat. |
...
| Blue-potion (full HP restore) | ✓ FAQ-CORRECTED | Per FAQ §3.3.2: blue potion = **full HP restore**, NOT invulnerability.  Functionally equivalent to Juu-en Fruit (item 1 = use_hp_full).  Found in chests + buried in walls (final-world walls have many UP-buried blue potions reached by jumping + slashing upward).  Healing is FREE (vs paid Juu-en Fruit at 200 gold).  Red Potion = ~80HP (like Ken'ko); Blue = full.  Earlier "REFUTED as invul" was right about no invul mechanic, but wrong to imply blue potion doesn't exist — it does, as a healing pickup, NOT a use-item. |
...
| Power Shield (cheat-only "shield refill") | ✓ FAQ-CONFIRMED | Per FAQ §8.2.2: "The Power Shield is not really a shield.  I just makes the shield hit points go to maximum level.  It doesn't even have a picture.  With ZelTrain, you can get it with number 3."  This is a CHEAT-ONLY mechanic — not a real in-game pickup.  ZelTrain's key '3' writes `shield_HP = shield_max_HP` (DS:0x94 = DS:0x96).  No sprite, no inventory entry. |
| Enchantment Sword (secret) | ✓ FAQ-CONFIRMED | Per FAQ §8.2.1: secret sword (sword_type=6) obtained in Dorado via Lion's Head Key door — found alongside Feruza Shoes.  "Vanquishes any smaller foe with a single blow" — also kills Tumba's Red Slime (which is immune to regular swords).  Pickup mechanism documented at 200FIGHT.asm:6884: `mov sword, 6; mov bx, 18ABh; call drv_fn_14; mov al, 4; call sar_loader_fn` (loads new sword.grp sprite).  No purchase available in any shop — secret-only. |
...
| Recovery potion pickups (red = ~80HP, blue = full HP) | ✓ FAQ-CORRECTED | Red HP potion: 200FIGHT.asm:6954-6961 — `heal_pulse_count += (player_hp_max >> 3) + 1`; per-frame regen +8 HP/tick.  **Blue potion = full HP restore** per FAQ §3.3.2 (NOT invulnerability — earlier doc was wrong).  Blue likely uses a different entity-trigger address that sets heal_pulse_count to a larger value (max_HP * 8) or writes player_HP = player_hp_max directly.  Final-world (Pureza) has many blue potions buried in upper walls (reach by jumping + slashing upward). |
...
| F1 (music toggle) | ✓ FAQ-CORRECTED | scan 0x3B → bit 0x1000.  `handle_special_keys` (stick.asm:269-275): when bit set + held → `mov gvar_volume_b, 1; mov cl, gvar_key_pressed; mov ax, 2; int 60h`.  INT 60h AX=2 is the **mscmt.drv music-toggle service**.  Confirmed by FAQ §11 "Use F1 to toggle music on/off".  Earlier "F1 = skip-key" was wrong; FAQ is authoritative. |
| F2 (SFX toggle) | ✓ FAQ-CORRECTED | scan 0x3C → bit 0x2000.  `handle_special_keys` (stick.asm:286-292): when bit set → `not gvar_sound_flag`.  Controls SFX path only (music has separate state via INT 60h).  Confirmed by FAQ §11 "Use F2 to toggle sound FX on/off".  F1 + F2 are independent toggles. |
...
| Music-on/off F1 | ✓ FAQ-CONFIRMED | See §14 — F1 IS the music toggle (INT 60h AX=2 = mscmt.drv music-toggle service); my earlier flip was wrong, FAQ §11 is authoritative |
| SFX-on/off F2 | ✓ FAQ-CONFIRMED | See §14 — F2 IS the SFX toggle (`not gvar_sound_flag`); independent from music; FAQ §11 confirms |
...
| HP regen at safe spots (FAQ §10) | ✓ FAQ-CONFIRMED | "If you stand still, you will regain your strength" (FAQ §10).  Per-frame `heal_pulse_count` decrements at 200FIGHT.asm:2941-2952 when not in combat (no scroll, no active fight FSM, no enemy nearby).  `heal_pulse_count > 0` triggers `player_HP += 8` per tick until clamped at `player_hp_max` (line 2947-2952).  Standing still avoids spawning new combat ticks and lets the regen drain.  Sword swing while standing is documented separately as defense. |
| Save-game-only command-line load | ✓ FAQ-CONFIRMED | Per FAQ §8.5: `zeliad.exe <savename>` skips opening cinematic + title and jumps straight to gameplay.  Mechanism in zeliad.asm: `has_savefile` byte set if cmdline arg present; `cbw` converts to 0/0xFFFF in AX; `jmp dword ptr cs:game_entry_ofs` passes AX to game.bin which branches on `save_mode_flag == 0xFFFF` (load-mode path at game.asm:278-290).  Example: `zeliard alan` loads `alan.usr`. |
| ZelEdit (hex save editor) | ✓ FAQ-CONFIRMED | External tool that edits .usr save files.  Per FAQ §9.1: edits sword/shield/keys/crests/spells/HP/items via offset table at cheatingdome.com/pc/2710p.htm.  Operates on the 256-byte .USR file directly (per SAVE_FORMAT.md byte map).  Not a game-runtime mechanic. |
| ZelTrain (in-game memory trainer) | ✓ FAQ-CONFIRMED | External TSR-like trainer.  Per FAQ §9.2 — keybindings: 1=better sword, 2=better shield, 3=max shield HP, 4=keys (up to 9), 5=Lion Key (up to 9), 6=Hero's Crest toggle, 7=Glory Crest, 8=Elf Crest, 9=add+max spell, 0=add wear item, `-`=add use item, `=`=999,9999 gold.  Operates on game memory at runtime (writes player record bytes at DS:0x80..0xC1). |
| Sword swing as defense (stand-and-swing) | ✓ FAQ-CONFIRMED | See §4 Combat — `Up+Down+Space` keeps player grounded while swinging; combined with HP regen at safe spots, this allows wait-and-attack tactic for tough enemies. |
...
**Total mechanics enumerated**: 239
**Coverage**: 100% fully understood, 0% partial, 0% not investigated

**🎯 2026-05-11 FAQ-cross-reference pass**: Added 10 new mechanic rows
verified against `4_Resources/FAQ.html` (Alan Franciškovic's GameFAQs
walkthrough, 2001).  Corrections:
- F1 = music toggle (NOT skip-key — third correction, FAQ-authoritative)
- F2 = SFX toggle (separate from music)
- Blue Potion = full-HP heal (NOT invul — both my "REFUTED" and "no such item" claims were partially wrong)

New rows: Sword swing direction variants, Sword as defense, Gelroid
HP-drain, Power Shield (cheat-only), Enchantment Sword (secret),
HP regen at safe spots, Command-line save load, ZelEdit / ZelTrain
external tools.
