# ZELRES2 Extended Chunks Reference (40–57)

**Archive**: `zelres2.sar`
**Extended chunk range**: 40–57 (18 chunks; chunk_50 is code — see 250GMENG.asm)
**Location in SAR**: Extended offset table at bytes 0xA0–0xE7 (18 × 4-byte entries)
...
These chunks are accessed via the SAR extended offset table — a second set of 4-byte offsets
...
- **NPC data** (40–45): multi-section format — NEC PC-98 section + optional VGA section
- **Music** (46–49): raw AL=3 load — MSD sequencer format, flag byte is first data byte
- **Graphics** (51–57): fill_buffer compressed GRP images (various opcodes)

---

## Chunk 40 — `npc_nec40.bin` (3,588 bytes)

**Type**: NEC PC-98 NPC behavior data (no VGA equivalent)
**Format**: Multi-section; NEC section only — dense 0xC7/0xC8 byte patterns
**Purpose**: Town NPC behavior tables for NEC PC-98 hardware. No VGA content means
this data is skipped on VGA/DOS platforms.

---

## Chunk 41 — `npc_dlg41.bin` (4,082 bytes)

**Type**: Bilingual NPC dialogue
**Format**: Multi-section — NEC section (Japanese, 3784 bytes) + VGA section (English, ~289 compressed → ~167 bytes)
...
## Chunks 42–45 — NEC PC-98 NPC Data
...
## Chunks 46–49 — Music Sequences (MSD Format)

**Format**: Raw AL=3 load. MSD (Music Sequence Data) — Zeliard's proprietary music format.
**Magic identifier**: bytes 12–15 = `01 1F 00 02` in all MSD files.
**Sequencer opcodes**: `0xF0, 0xE9, 0xE5, 0xE6, 0xE2` = Zeliard music events.

| Chunk | File | Size | Track |
|-------|------|------|-------|
| 46 | mgt1.msd | 3,565B | MGT1 — Town BGM track 1 |
| 47 | mgt2.msd | 3,292B | MGT2 — Town BGM track 2 |
| 48 | ugm1.msd | 4,042B | UGM1 — Underground/dungeon BGM 1 |
| 49 | ugm2.msd | 1,784B | UGM2 — Underground/dungeon BGM 2 (short) |

---

## Chunk 50 — CODE (250GMENG.asm)

**See**: `3_Assembly/tasm/working/zelres2/code/250GMENG.asm`
...
## Chunk 51 — `chunk_51.bin` (4,904 bytes → 10,720 decompressed)
