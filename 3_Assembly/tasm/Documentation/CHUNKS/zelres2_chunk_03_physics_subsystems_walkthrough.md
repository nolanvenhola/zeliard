...
|---------|---------------|
| `0x0000` | `zr2_03` |
| `0x011a` | `physsub_func_33` |
| `0x0128` | `physsub_get_value_2` |
| `0x0270` | `physsub_func_30` |
...
**Purpose**: Identical logic to chunk_04, but calls the CGA animation handler at physsub_multiply (0x422) instead of 0x327.
...
**Plain English**: 3 more standard tiles plus 1 special tile (offset 9) that uses a different handler at copy_buffer (0x4a3). This handler likely deals with moving platforms, doors, or boss triggers.
...
**Plain English**: Walk through the visual tile map (SI) and collision buffer (DI). Use `cmpsb` to detect changes. When a tile changes (e.g., switch flipped, platform moved), call physsub_get_value_2 (0x128) to update both the collision byte and re-render the tile in CGA mode. This keeps collision and visuals synchronized.
...
## Section 4: CGA Sprite Blitter (physsub_func_30 (0x0270)-0x0421)

### Subsection 4A: CGA Address Calculation & Sprite Load (physsub_func_30 (0x0270)-0x02CD)
...
## Section 5: Moving Platform System (physsub_process_loop (0x0684)-0x080D)

### Subsection 5A: Platform State Handler (copy_buffer (0x04a3)-0x04DA)
...
### Subsection 5B: Platform Renderer (physsub_process_loop (0x0684)-0x0765)
