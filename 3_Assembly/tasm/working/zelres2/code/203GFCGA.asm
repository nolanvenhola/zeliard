...
; CGA driver init code (parallel to 202GFEGA: push cs/pop es, clear sprite_cache_tbl,
; drv_init_stub patchable byte, mov word [vga_row_ptr], call via cga_row_ofs patch,
; sprite-scan loop with sprite_slot_remove call). See 202GFEGA for full mnemonic decode.
		db	 0Eh, 07h,0BFh, 9Fh, 50h	; push cs / pop es / mov di, 509Fh (sprite_cache_tbl)
		db	 33h,0C0h,0B9h, 80h, 00h	; xor ax,ax / mov cx, 80h
		db	0F3h,0ABh			; rep stosw  (clear sprite_cache_tbl, 0x80 words)
		db	0FEh, 06h, 80h, 50h		; drv_init_stub: inc byte ptr [5080h] (anim_phase) -- patchable
		db	0C7h, 06h, 6Dh, 50h, 3Ch, 02h	; mov word ptr [506Dh], 023Ch  (vga_row_ptr)
		db	 8Bh, 36h, 31h,0FFh		; mov si, [0FF31h]  (sprite_data_ptr)
		db	 83h,0EEh, 21h			; sub si, 21h
		db	0E8h, 8Bh, 15h			; cga_row_ofs: call (E8) with patchable disp -- initially calls 15DBh
		db	 33h,0DBh			; xor bx, bx
		db	0F6h, 04h, 80h			; test byte ptr [si], 80h
		db	 74h, 03h			; jz +3 (skip sprite_slot_remove call)
		db	0E8h,0C8h, 03h			; call 0422h  (sprite_slot_remove equivalent)
		db	 46h				; inc si
...
loc_114:
		add	si,4
		jmp	loc_110
; Dead bytes between functions — Sourcer mis-decoded as code (no entry point).
; These 9 bytes precede the 16-byte zero pad before the sprite/bitmap data block.
		db	 45h, 3Ah, 05h, 3Ah,0C5h, 39h, 85h, 39h, 00h
		db	16 dup (0)
