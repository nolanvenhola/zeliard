...
locloop_52:
		...
		mov	dx,12AEh
		db	08h, 0F0h			; or al,dh (alt encoding: 08 r/m8,r8 not 0A r8,r/m8)
		...
		add	al,[bx+di-5Eh]
		db	82h, 41h, 0A0h, 22h		; add byte ptr [bx+di-60h],22h (alt encoding: 82/0 not 80/0)
		adc	[bp+si],sp
		...
		db	82h, 8Ah, 11h, 02h, 8Ch		; or byte ptr [bp+si+211h],8Ch (alt encoding: 82/1 not 80/1)
		adc	[bp+di],sp
		inc	dx
		mov	al,ds:data_40e
		...
		call	$+412Dh
		test	al,80h
		db	11h, 0EAh			; adc dx,bp (alt encoding: 11 r/m16,r16 not 13 r16,r/m16)
		adc	ss:data_26e[bp+si],ax
		mov	ah,ds:data_57e[bx+si]
		db	82h, 41h, 2Ah, 20h		; add byte ptr [bx+di+2Ah],20h (alt encoding: 82/0 not 80/0)
		...
		mov	al,ds:data_59e
		inc	cx
		db	80h, 0C8h, 11h		; or al,11h (alt encoding: 80/1 ModRM not 0C short form)
		or	ch,[bx+si]
		mov	al,[bp+si-58h]
		mov	al,ds:data_44e[bx+si]
		db	0EAh, 82h, 0A2h, 41h, 12h	; jmp far (absolute; TASM won't compile as mnemonic)
			                        ;* No entry point to code
		or	ch,ds:data_28e[bx+si]
		inc	cx
		mov	[bx+si],cl
		db	0EAh, 11h, 88h, 80h, 0A8h	; jmp far (absolute; TASM won't compile as mnemonic)
			                        ;* No entry point to code
		...
		mov	al,2
		and	dl,[bx+di]
		db	0EAh, 0CAh, 42h, 8Ah, 8Bh	; jmp far (absolute; TASM won't compile as mnemonic)
