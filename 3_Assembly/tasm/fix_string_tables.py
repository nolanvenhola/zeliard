...
def find_asm_lines_for_range(lst_text, src_lines, bin_start, bin_end):
    ...
    # Find ASM indices in range
    indices = [idx for o, idx in off_to_asm.items()
               if bin_start <= o < bin_end]
    if not indices:
        return None, None
...
if not DRY_RUN and files_modified:
    print('\nRebuilding to verify bit-perfect output...')
