...
def probe(h, input_ax, scroll_col, map_w):
    h.write_word(0x80, scroll_col)
    h.write_word(0xC002, map_w)
    snap = h.snapshot()
    result = h.call_function(GAME_FUNC_87,
...
def main() -> int:
    ...
    print(f'game_func_87 @ CPU 0x{GAME_FUNC_87:04X}')

    # Same shape as game_func_141 but constant is 0x21
    expect_a = 0x21 - (20 - 10)
    got_a = probe(h, 20, 10, 40)
    a_ok = got_a == expect_a
