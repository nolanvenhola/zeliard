...
e:\link zeliard.obj, zeliard.exe /CPARMAXALLOC:513; >>log.txt
...
{ cmp -l ../../1_OriginalGame/zeliad.exe ZELIARD.EXE | gawk '{printf "0x%08X: %02X %02X\n", $1-1, strtonum(0$2), strtonum(0$3)}'; } >>diff.txt 2>&1
...
{ cmp -l ../../1_OriginalGame/game.bin game.bin | gawk '{printf "0x%08X: %02X %02X\n", $1-1, strtonum(0$2), strtonum(0$3)}'; } >>diff.txt 2>&1
echo "mole.bin diffs:" >>diff.txt
{ cmp -l ../../2_SAR/VFSExtractor/2/mole.bin mole.bin | gawk '{printf "0x%08X: %02X %02X\n", $1-1, strtonum(0$2), strtonum(0$3)}'; } >>diff.txt 2>&1
...
{ cmp -l ../../2_SAR/VFSExtractor/1/gdmcga.bin gdmcga.bin | gawk '{printf "0x%08X: %02X %02X\n", $1-1, strtonum(0$2), strtonum(0$3)}'; } >>diff.txt 2>&1
...
{ cmp -l ../../2_SAR/VFSExtractor/2/select.bin select.bin | gawk '{printf "0x%08X: %02X %02X\n", $1-1, strtonum(0$2), strtonum(0$3)}'; } >>diff.txt 2>&1
