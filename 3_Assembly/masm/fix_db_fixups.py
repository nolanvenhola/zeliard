...
# TASM format:  "   1413\t0CE2  83 3C FF\t\t\t     db  83h, ..."
# MASM format:  " 0CE2  83 3C FF                   db  83h, ..."
_LST_BYTE_LINE_TASM = re.compile(
    r'^\s+(\d+)\s+([0-9A-F]{4})\s+((?:[0-9A-F]{2,4}\s+)*)',
)
_LST_BYTE_LINE_MASM = re.compile(
    r'^ ([0-9A-F]{4})\s+((?:[0-9A-F]{2,4}\s+)*)',
)

def _parse_lst_byte_line(line, seq):
    """Try TASM then MASM LST format. Returns (lst_lnum, offset, hex_tokens) or None."""
    m = _LST_BYTE_LINE_TASM.match(line)
    if m:
        return int(m.group(1)), int(m.group(2), 16), m.group(3).split(), m.end()
    m = _LST_BYTE_LINE_MASM.match(line)
    if m:
        return seq, int(m.group(1), 16), m.group(2).split(), m.end()
    return None
...
_LST_PROC_LBL_MASM = re.compile(
    r'^ [0-9A-F]{4}\s+\S+\s+(\w+)\s+(proc|label)\b', re.I
)
...
def parse_lst_fixups(lst_text: str):
    ...
    fixups       = []
    offset_table = []
    seq = 0  # sequential counter for MASM format (no line numbers)
    ...
    for line in lst_text.split('\n'):
        ...
        # Lines with byte dumps (TASM or MASM format)
        parsed = _parse_lst_byte_line(line, seq)
        if not parsed:
            continue

        lst_lnum, offset, hex_tokens, end_pos = parsed
        rest = line[end_pos:].strip()
...
def main():
    ...
    # Note: MASM 4.0 cannot compile the largest files (memory limits in DOSBox).
    # Verification is done via tasm/build_all.py --verify (TASM build is canonical).
    if total > 0 and not dry_run:
        print('\nNote: run tasm/build_all.py --verify for bit-perfect verification.')
