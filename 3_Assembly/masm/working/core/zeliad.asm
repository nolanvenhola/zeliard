...
save_name_done:
		...
		jz	load_gfx_driver
		mov	di,offset entry_cmdline_savefile
...
load_gfx_driver:
		...
		add	ax,0FF0h
		mov	es,ax
		mov	di,offset entry_music_driver
		call	load_driver_file
		...
		add	ax,0FF0h
		mov	es,ax
		mov	di,offset entry_joystick_driver
		call	load_driver_file
...
;==========================================================================
;  Runtime Variables - Driver entry structures
;  Each entry: { load_offset:word, filename:string_buffer }
;  load_driver_file is called with DI = &entry; opens filename at [DI+2]
;  and reads file contents to ES:[load_offset].
;==========================================================================

entry_cmdline_savefile:
		dw	0			; load_offset = 0 (load at ES:0)
cmdline_savefile:				; filename buffer (32 bytes)
		db	0
		db	31 dup (0)

entry_music_driver:
		dw	0100h			; load_offset = 0x0100 (music seg)
music_driver_name:				; filename buffer (16 bytes)
		db	0
		db	15 dup (0)

entry_joystick_driver:
		dw	1100h			; load_offset = 0x1100 (sfx seg)
joystick_driver_name:				; filename buffer (16 bytes)
		db	0
		db	15 dup (0)
