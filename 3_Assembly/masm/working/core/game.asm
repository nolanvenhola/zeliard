...
DEBUG_BUILD	EQU	0
; game.bin is directly loaded at CS:0xA000 (no SAR 4-byte header strip).
; DBG_CHUNK_BASE = CHUNK_LOAD_BASE for directly-loaded files.
CHUNK_LOAD_BASE	EQU	0A000h
DBG_CHUNK_BASE	EQU	CHUNK_LOAD_BASE
...
MASM_MOV_MEM16P2_AX  MACRO  addr_equ
        DB 0A3h           ; MOV [m16],AX short form (A3)
        DW addr_equ + 2
ENDM
MASM_CALL_FAR_MEM  MACRO  addr_equ
        ...
        DW addr_equ
ENDM
MASM_MOV_CS_WORD_AX  MACRO  addr_equ
        DB 2Eh, 0A3h       ; CS: MOV [m16],AX (A3 short form)
        DW addr_equ
ENDM
...
start:
		MASM_MOV_CS_WORD_AX save_mode_flag	; mov cs:[save_mode_flag], ax
		mov	ax,cs
		mov	ds,ax
		push	cs
		pop	es
		...
		MASM_CMP_CS_WORD_NEG1 save_mode_flag	; cmp word ptr cs:[save_mode_flag], -1
		jz	start_load_game
...
start_load_game:
		...
		mov	ds,ax
		add	ax,3000h
		MASM_MOV_MEM16P2_AX game_init_fn	; mov word ptr ds:[game_init_fn+2], ax
		mov	es,ax
		...
		push	ds
		MASM_CALL_FAR_MEM game_init_fn		; call dword ptr ds:[game_init_fn]
		pop	ds
...
; ============================================================
; DEBUG HELPER PROCEDURES + STRING POOL (no-op when DEBUG_BUILD EQU 0)
; ============================================================
		DBG_PROCS

IF DEBUG_BUILD
dbg_g_init		db	'GAME:INIT', 0
dbg_g_font_ok		db	'font.grp ok', 0
dbg_g_gfx_ok		db	'gfx drv ok', 0
dbg_g_opdemo_load	db	'loading opdemo...', 0
dbg_g_opdemo_ok		db	'opdemo ok', 0
ENDIF
