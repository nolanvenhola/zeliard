...
; Set DEBUG_BUILD to 1 to enable real-time trace via DOSBox-X port 0E9h.
; Set to 0 (default) for normal bit-perfect builds.
DEBUG_BUILD	EQU	0
SIZE_TEST	EQU	0	; set to 1 to append a single 00h byte (tests if chunk size change alone causes DISK1 error)
...
CHUNK_LOAD_BASE		equ	6000h		; chunk is loaded at game_seg:6000h by LOAD_CHUNK in game.asm
DBG_CHUNK_BASE		EQU	5FFCh		; runtime base = CHUNK_LOAD_BASE - 4 (SAR strips 4-byte header)
					; runtime CS address = CHUNK_LOAD_BASE + offset label
...
res_nec_grp	equ	DBG_CHUNK_BASE + offset ref_nec_grp	; 953Dh — zelres1 ch23 nec.grp
res_hou_grp	equ	DBG_CHUNK_BASE + offset ref_hou_grp	; 9547h — zelres1 ch18 hou.grp
res_dmaou_grp	equ	DBG_CHUNK_BASE + offset ref_dmaou_grp	; 9551h — zelres1 ch15 dmaou.grp
res_zopn_msd	equ	DBG_CHUNK_BASE + offset ref_zopn_msd	; 955Dh — zelres1 ch40 zopn.msd
res_ttl1_grp	equ	DBG_CHUNK_BASE + offset ref_ttl1_grp	; 9568h — zelres1 ch30 ttl1.grp
res_ttl2_grp	equ	DBG_CHUNK_BASE + offset ref_ttl2_grp	; 9573h — zelres1 ch31 ttl2.grp
res_ttl3_grp	equ	DBG_CHUNK_BASE + offset ref_ttl3_grp	; 957Eh — zelres1 ch32 ttl3.grp
res_zend_msd	equ	DBG_CHUNK_BASE + offset ref_zend_msd	; 9589h — zelres1 ch39 zend.msd
res_waku_grp	equ	DBG_CHUNK_BASE + offset ref_waku_grp	; 9594h — zelres1 ch33 waku.grp
res_ame_grp	equ	DBG_CHUNK_BASE + offset ref_ame_grp	; 959Fh — zelres1 ch14 ame.grp
res_hime_grp	equ	DBG_CHUNK_BASE + offset ref_hime_grp	; 95A9h — zelres1 ch16 hime.grp
res_isi_grp	equ	DBG_CHUNK_BASE + offset ref_isi_grp	; 95B4h — zelres1 ch19 isi.grp
res_oui_grp	equ	DBG_CHUNK_BASE + offset ref_oui_grp	; 95BEh — zelres1 ch26 oui.grp
res_sei_grp	equ	DBG_CHUNK_BASE + offset ref_sei_grp	; 95C8h — zelres1 ch28 sei.grp
res_yuu1_grp	equ	DBG_CHUNK_BASE + offset ref_yuu1_grp	; 95D2h — zelres1 ch34 yuu1.grp
res_yuu2_grp	equ	DBG_CHUNK_BASE + offset ref_yuu2_grp	; 95DDh — zelres1 ch35 yuu2.grp
res_yuu3_grp	equ	DBG_CHUNK_BASE + offset ref_yuu3_grp	; 95E8h — zelres1 ch36 yuu3.grp
res_yuu4_grp	equ	DBG_CHUNK_BASE + offset ref_yuu4_grp	; 95F3h — zelres1 ch37 yuu4.grp
res_yuup_grp	equ	DBG_CHUNK_BASE + offset ref_yuup_grp	; 95FEh — zelres1 ch38 yuup.grp
res_oup_grp	equ	DBG_CHUNK_BASE + offset ref_oup_grp	; 9609h — zelres1 ch27 oup.grp
res_maop_grp	equ	DBG_CHUNK_BASE + offset ref_maop_grp	; 9613h — zelres1 ch20 maop.grp
res_game_bin	equ	DBG_CHUNK_BASE + offset ref_game_bin	; 961Eh — game.bin restart target
...
post_title_story_scenes:
		DBGSTR	dbg_s_post_title
		DBGNL
		RESET_STACK
		mov	byte ptr cs:[gvar_spacebar_state],0
		mov	byte ptr cs:[gvar_enter_key],0
		mov	word ptr cs:[script_pc],CHUNK_LOAD_BASE + offset narration_prologue
		mov	ax,5
		call	word ptr cs:[gfx_palette_fn]
		DBGSTR	dbg_s_ld06
		DBGNL
		LOAD_DATA res_waku_grp, vga_seg
		DBGSTR	dbg_s_ld06_ok
		DBGNL
		SET_ES_2000
		mov	si,vga_seg
		mov	di,0
		...
		call	word ptr cs:[gfx_palette_fn]
		BLIT_SCENE_FRAME
		LOAD_DATA res_hime_grp, vga_seg
		DECOMPRESS_VGA scene_framebuf
		...
		LOAD_DATA res_dmaou_grp, vga_seg
		DECOMPRESS_VGA scene_data_i
		DBGSTR	dbg_s_call03	; P12 rain of sand
		call	run_script_interpreter
		mov	al,4
		...
		BLIT_SCENE_FRAME
		DBGSTR	dbg_s_call04	; P14 king wept
		call	run_script_interpreter
		DBGSTR	dbg_s_call05	; P15 tears awakened
		call	run_script_interpreter
		SET_ES_2000
		mov	di,0
		mov	bx,1728h
		...
		mov	cx,4868h
		call	word ptr cs:[gfx_mode_fn]
		DBGSTR	dbg_s_call08	; P22 spirit vanished
		call	run_script_interpreter
		mov	ax,7
		call	word ptr cs:[gfx_palette_fn]
		GFX_BLIT 410h, 4868h, 4000h
		DBGSTR	dbg_s_call09	; P24 duke arrives
		call	run_script_interpreter
		...
		mov	di,scene_framebuf
		call	word ptr cs:[gfx_update_fn]
		DBGSTR	dbg_s_call10	; P26 duke escorted
		call	run_script_interpreter
		DBGSTR	dbg_s_call11	; P27 king animated speech
		call	run_script_interpreter
		...
		LOAD_DATA res_yuup_grp, vga_seg
		DECOMPRESS_VGA scene_framebuf
		LOAD_DATA res_oup_grp, vga_seg
		DECOMPRESS_VGA screen_buf_1
		DBGSTR	dbg_s_call14	; P33 jashiin labyrinths
		call	run_script_interpreter
		...
		DBGSTR	dbg_s_call18	; P40 jashiin speech
		call	run_script_interpreter
		DBGSTR	dbg_s_call19	; extra (scroll loop section)
		call	run_script_interpreter
		mov	bx,1515h
		mov	dx,315Dh
		mov	cx,18h
...
gameplay_input_loop:
		...
		mov	ax,1
		call	word ptr cs:[gfx_palette_fn]
		mov	si,CHUNK_LOAD_BASE + offset anim_fade_tbl_scene
...
animate_scanline_alt		endp
		...
		dw	CHUNK_LOAD_BASE + offset narration_prologue	; initial script_pc
...
merge_gfx_planes		endp
		...
		db	'           Two thousand years, ', CR, 'from the dark reaches of another galaxy,', CR
		db	'        a demon with not a shred', CR
		db	'      of compassion for humankind,', CR, '         descended upon earth.', CR, CR, '          He defiled the land,', CR
		db	'  sending vile creatures to live in it,', CR, '   and thus became ruler of the world.', CR, CR
		db	'         The King of Felishika,', CR
		db	'     appalled by what had happened,', CR, '          prayed to the Spirit', CR, '      of the Holy Land of Zeliard', CR
		db	'    for help in defeating this monster.', CR, CR, '    With the help of the holy crystals', CR
		db	'       called Tears of Esmesanti,', CR
		db	'    the King managed to wrest power', CR, '    from the fiend and seal him deep', CR
		db	'     within the bowels of the earth.', CR
		db	CR, '            And once again,', CR, ' the light of peace came to shine upon', CR, '              the earth.', CR
		db	CR, CR, 'However, it is written in', CR, '       the Sixth Book of Esmesanti:', CR
		db	'                    The Age of Darknes'
anim_fade_tbl_scene:
		db	's.', CR
		db	SCR_END_SCRIPT, ' '
		db	'               At last,                ', CR, '     the door of destiny was opened.    ', CR
		db	'        The labyrinths are deep,        ', CR
		db	'          and the way is long.          ', CR, '     Will Duke Garland be successful    ', CR
		db	'   in dethroning the Emperor of Chaos?'
anim_fade_tbl_credits:
		db	'  ', CR
		db	SCR_END_SCRIPT, ' '
		db	'         Fantasy Action Game           ', CR, '               ZELIARD                  ', CR, CR
		db	'             -- STAFF --                ', CR
		db	CR, 'Producer -- Japanese Version', CR, '                      Mitsuhiro Mazda   ', CR, CR
		db	'Producer -- English Version', CR
		db	'                        Josh Mandel     ', CR, CR, 'Lead Programmer      Tomoyuki Shimada   ', CR, CR
		db	'Graphic Designers     Akihiko Yoshida   ', CR
		db	'                      Masatoshi Azumi   ', CR, CR, 'English Text Translation by', CR
		db	'                       Marti McKenna    ', CR
		db	CR, 'Music Composers  -- MECANO ASSOCIATES --', CR, '                    Fumihito Kasatani   ', CR
		db	'                    Nobuyuki Aoshima    ', CR
		db	CR, 'Story Maker           Masaru Takeuchi   ', CR, CR, 'Sound Effects by     Tomoyuki Shimada   ', CR, CR
		db	'Advisers               Osamu Harada     ', CR, '                       Hiromi Ohba      ', CR
		db	'                       Greg Miyaji      ', CR
		db	CR, 'System Designer      Rocky Cave Maker   ', CR, CR, 'Special Thanks to', CR
		db	'                    Toshiyuki Uchida    ', CR
		db	'                       Yuzo Sunaga      ', CR, '                     Takeshi Miyaji     ', CR
		db	'                     Naozumi Honma      ', CR
		db	'                     Toshi Masubuchi    ', CR, '                     Ray E. Nakazato    ', CR
		db	'                     Hiroyuki Koyama    ', CR
		db	'                     Satoshi Uesaka     ', CR, '              Sierra On-Line Japan, Inc.', CR
		db	'                    Eiji (Ed) Nagano    ', CR
		db	CR, CR, CR, '    Copyright (C)1987,1990 GAME ARTS    ', CR, '    Copyright (C)1990 Sierra On-Line    ', CR
		db	'  This edition first published 1987 by  ', CR, '  GAME ARTS Co.,Ltd./ Tomoyuki Shimada'
narration_prologue:
		db	'  ', CR
...
dialogue_scene_start:
		...
		db	'"I am Jashiin, the Emperor of Chaos.  The descendants of those who imprisoned me under the earth shall know that '
		db	'my wrath has smoldered for two thousand years!"'
		db	SCR_RESET				; reset style
		db	SCR_WAIT				; pause
		db	SCR_WAIT				; pause
		db	SCR_BREAK				; end of section
...
narration_chapter_2:
		...
		db	SCR_BOLD				; bold text
		db	'"Oh, my beloved Felicia!  I fear the Age of Darkness is upon us.  I am powerless to stop it ... and powerless to '
		db	'help you."'
		db	SCR_RESET				; reset style
		db	SCR_WAIT				; pause
		db	SCR_WAIT				; pause
		...
		db	SCR_DIRECT				; layout: direct
		db	SCR_BOLD				; bold text
		db	'"I am the Guardian Spirit of the Holy Land of Zeliard.  The demon Jashiin has been resurrected, and indeed his ev'
		db	'il magic will plunge this world into the Age of Darkness once again."'
		db	SCR_WAIT				; pause
		db	SCR_WAIT				; pause
		db	SCR_WAIT				; pause
		db	SCR_SCROLL				; scroll text up
		db	SCR_DIRECT				; layout: direct
		db	'"Heed my words, King Felishika: There is but one way to stop this demon.  A brave warrior must venture into the l'
		db	'abyrinths and recover the nine Holy Crystals, the Tears of Esmesanti."'
		db	SCR_WAIT				; pause
		db	SCR_WAIT				; pause
		db	SCR_WAIT				; pause
		db	SCR_WAIT				; pause
		...
		db	SCR_DIRECT				; layout: direct
		db	'"'
		db	'You must await the arrival of this brave and noble knight, and tell him everything.  Only with his help can you h'
		db	'ope to restore this land to its former beauty, and free your daughter from her terrible curse."'
		db	SCR_WAIT				; pause
		db	SCR_WAIT				; pause
		db	SCR_WAIT				; pause
		db	SCR_WAIT				; pause
...
narration_chapter_3:
		...
		; pause | pause | speaker: King Felishika (attr >) | scroll-text-up | layout-mode 1
		db	SCR_WAIT,SCR_SPK_KING,SCR_SCROLL,SCR_PARA, '"', ANIM_83
		; Style-encoded speech -- King Felishika
		...
		db	SCR_WAIT				; pause
		; scroll-text-up | layout-mode 1 | text-style: color 7 bold | speaker: unknown (attr =)
		db	SCR_SCROLL,SCR_PARA,SCR_BOLD,SCR_SPK_UNK, ANIM_9A, '"'
...
narration_chapter_4:
		...
		db	SCR_DIRECT				; layout: direct
		db	22h				; 22h
		db	'Young fool, I could destroy you now, but I need a little amusement.  I will give you some time to perform your li'
		db	'ttle quest, but you must promise not to bore me."'
		db	SCR_WAIT				; pause
		db	SCR_WAIT				; pause
		db	SCR_WAIT				; pause
		db	SCR_SCROLL				; scroll text up
...
disp_narr_chap2		db	'u', ' '	; = dw 2075h (function pointer into stick.bin)
disp_chap2_call		db	'h', 'a'
disp_drv_seg_3		db	'v', 'e'
disp_narr_chap3		db	'n', 27h	; = dw 276Eh (function pointer into stick.bin)
disp_narr_open		db	't', ' '	; = dw 2074h (function pointer into stick.bin)
...
disp_script_area		db	' ', 'l'
		db	'ast of m'
disp_narr_chap4	db	'e', ','	; = dw 2C65h (function pointer into stick.bin)
		db	' Jashiin!'
		db	SCR_MODE2		; layout-mode 2
...
ref_nec_grp	db	00h, 17h, 'nec.grp',   0	; zelres1 chunk 23
ref_hou_grp	db	00h, 12h, 'hou.grp',   0	; zelres1 chunk 18
ref_dmaou_grp	db	00h, 0Fh, 'dmaou.grp', 0	; zelres1 chunk 15
ref_zopn_msd	db	00h, 28h, 'zopn.msd',  0	; zelres1 chunk 40 (opening music)
ref_ttl1_grp	db	00h, 1Eh, 'ttl1.grp',  0	; zelres1 chunk 30
ref_ttl2_grp	db	00h, 1Fh, 'ttl2.grp',  0	; zelres1 chunk 31
ref_ttl3_grp	db	00h, 20h, 'ttl3.grp',  0	; zelres1 chunk 32 (Zeliard logo)
ref_zend_msd	db	00h, 27h, 'zend.msd',  0	; zelres1 chunk 39 (ending music)
ref_waku_grp	db	00h, 21h, 'waku.grp',  0	; zelres1 chunk 33
ref_ame_grp	db	00h, 0Eh, 'ame.grp',   0	; zelres1 chunk 14
ref_hime_grp	db	00h, 10h, 'hime.grp',  0	; zelres1 chunk 16
ref_isi_grp	db	00h, 13h, 'isi.grp',   0	; zelres1 chunk 19
ref_oui_grp	db	00h, 1Ah, 'oui.grp',   0	; zelres1 chunk 26
ref_sei_grp	db	00h, 1Ch, 'sei.grp',   0	; zelres1 chunk 28
ref_yuu1_grp	db	00h, 22h, 'yuu1.grp',  0	; zelres1 chunk 34
ref_yuu2_grp	db	00h, 23h, 'yuu2.grp',  0	; zelres1 chunk 35
ref_yuu3_grp	db	00h, 24h, 'yuu3.grp',  0	; zelres1 chunk 36
ref_yuu4_grp	db	00h, 25h, 'yuu4.grp',  0	; zelres1 chunk 37
ref_yuup_grp	db	00h, 26h, 'yuup.grp',  0	; zelres1 chunk 38
ref_oup_grp	db	00h, 1Bh, 'oup.grp',   0	; zelres1 chunk 27
ref_maop_grp	db	00h, 14h, 'maop.grp',  0	; zelres1 chunk 20
ref_game_bin	db	00h, 00h, 'game.bin',  0	; game main binary

IF SIZE_TEST
		db	0			; SIZE_TEST padding byte
ENDIF

; ============================================================
; DEBUG HELPER PROCEDURES + STRING POOL
; Placed at the END so they don't shift any existing data labels.
; All debug strings use offsetto compute
; their runtime CS address. (no-op when DEBUG_BUILD EQU 0)
; ============================================================
...
dbg_s_ld02		db	'LD02:nec ', 0
dbg_s_ld02_ok		db	'ok', 0
dbg_s_ld03		db	'LD03:dmaou ', 0
dbg_s_ld03_ok		db	'ok', 0
dbg_s_ld04		db	'LD04:ttl1 ', 0
dbg_s_ld04_ok		db	'ok', 0
dbg_s_ld05		db	'LD05:ttl2 ', 0
dbg_s_ld05_ok		db	'ok', 0
dbg_s_ld06		db	'LD06:waku ', 0
dbg_s_ld06_ok		db	'ok', 0
dbg_s_ld07		db	'LD07:ame ', 0
dbg_s_ld07_ok		db	'ok', 0
dbg_s_post_title	db	'POST_TITLE', 0
dbg_s_call01		db	'CALL01 ', 0
dbg_s_call02		db	'CALL02 ', 0
dbg_s_call03		db	'CALL03 ', 0
dbg_s_call04		db	'CALL04 ', 0
dbg_s_call05		db	'CALL05 ', 0
dbg_s_call06		db	'CALL06 ', 0
dbg_s_call07		db	'CALL07 ', 0
dbg_s_call08		db	'CALL08 ', 0
dbg_s_call09		db	'CALL09 ', 0
dbg_s_call10		db	'CALL10 ', 0
dbg_s_call11		db	'CALL11 ', 0
dbg_s_call12		db	'CALL12 ', 0
dbg_s_call13		db	'CALL13 ', 0
dbg_s_call14		db	'CALL14 ', 0
dbg_s_call15		db	'CALL15 ', 0
dbg_s_call16		db	'CALL16 ', 0
dbg_s_call17		db	'CALL17 ', 0
dbg_s_call18		db	'CALL18 ', 0
dbg_s_call19		db	'CALL19 ', 0
dbg_s_call20		db	'CALL20 ', 0
dbg_s_transition	db	'TRANSITION_OUT', 0
dbg_s_scr_pc		db	'SCR+ PC=', 0
dbg_s_scr_end		db	' SCR_END', 0
dbg_s_scr_break		db	' SCR_BREAK', 0
ENDIF
