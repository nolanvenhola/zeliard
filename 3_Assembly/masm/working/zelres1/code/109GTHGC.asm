...
render_fn_tbl_a		equ	347Ah			;* render function pointer table A (word[N])	;* DBG: DBG_CHUNK_BASE + offset render_fn_tbl_a_lbl
render_fn_tbl_b		equ	34D6h			;* render function pointer table B (word[N])	;* DBG: DBG_CHUNK_BASE + offset render_fn_tbl_b_lbl
tile_row_data_a	equ	DBG_CHUNK_BASE + offset tile9_row_mask		;* tile row source data A
tile_row_data_b		equ	38D5h			;* tile row source data B
tile_type_tbl		equ	3B1Dh			;* tile type dispatch table (word[N])	;* DBG: DBG_CHUNK_BASE + offset tile_type_tbl_lbl
pixel_encode_tbl	equ	3C24h			;* pixel encoding lookup table	;* DBG: DBG_CHUNK_BASE + offset pixel_encode_tbl_lbl
tile_hgc_ofs		equ	3CACh			;* current HGC framebuffer byte offset for tile row (word)
tile_row_ctr		equ	3CAEh			;* current tile row counter (byte, 0..1Ch)	;* DBG: DBG_CHUNK_BASE + offset tile_row_ctr_lbl
tile_dest_ofs		equ	3CB0h			;* tile destination offset (word)
tile_col_ctr		equ	3CB5h			;* current tile column position counter (word)	;* DBG: DBG_CHUNK_BASE + offset tile_col_ctr_lbl
bitplane_w0		equ	3CB7h			;* bitplane word 0	;* DBG: DBG_CHUNK_BASE + offset bitplane_w0_lbl
bitplane_w1		equ	3CB9h			;* bitplane word 1	;* DBG: DBG_CHUNK_BASE + offset bitplane_w1_lbl
bitplane_w2		equ	3CBBh			;* bitplane word 2	;* DBG: DBG_CHUNK_BASE + offset bitplane_w2_lbl
bitplane_w3		equ	3CBDh			;* bitplane word 3	;* DBG: DBG_CHUNK_BASE + offset bitplane_w3_lbl
far_ptr			equ	3CBFh			;* far pointer dword (seg:ofs) used with LDS	;* DBG: DBG_CHUNK_BASE + offset far_ptr_lbl
...
tile_cache_tbl		equ	3F13h			;* tile HGC address cache table (word[N])	;* DBG: DBG_CHUNK_BASE + offset tile_cache_tbl_lbl
...
; ----------------------------------------------------------------------
tile_idx_a		equ	3CAFh			;* tile index A (byte)	;* DBG: DBG_CHUNK_BASE + offset tile_idx_a_lbl
tile_idx_b		equ	3CB2h			;* tile index B (byte, 0FDh = none)	;* DBG: DBG_CHUNK_BASE + offset tile_idx_b_lbl
