...
hgc_mask_tbl_a	equ	32BBh			;* First blit mask table (even passes)	;* DBG: DBG_CHUNK_BASE + offset hgc_mask_tbl_a_lbl
hgc_mask_tbl_b	equ	32CBh			;* Second blit mask table (odd passes)	;* DBG: DBG_CHUNK_BASE + offset hgc_mask_tbl_b_lbl
sprite_src_tbl	equ	3645h			;* Sprite source frame pointer table	;* DBG: DBG_CHUNK_BASE + offset sprite_src_tbl_lbl
sprite_frame_tbl	equ	3647h			;* Sprite animation frame index table	;* DBG: DBG_CHUNK_BASE + offset sprite_frame_tbl_lbl
move_seq_horiz	equ	3ACBh			;* DBG: DBG_CHUNK_BASE + offset move_seq_horiz_lbl
move_seq_up	equ	3B8Fh			;* Vertical (up) movement sequence data
color_pair_tbl	equ	3BC2h			;* Color pair lookup table	;* DBG: DBG_CHUNK_BASE + offset color_pair_tbl_lbl
hgc_palette_xlat	equ	44F5h			;* Palette translation table (HGC)
hgc_color_lut	equ	4C23h			;* Color lookup table (HGC)	;* DBG: DBG_CHUNK_BASE + offset hgc_color_lut_lbl
render_lut_ptr	equ	4C52h			;* Pointer into color lookup table for current render	;* DBG: DBG_CHUNK_BASE + offset render_lut_ptr_lbl
cur_col_ctr	equ	4C5Ch			;* Current column counter within blit pass	;* DBG: DBG_CHUNK_BASE + offset cur_col_ctr_lbl
cur_row_ctr	equ	4C5Dh			;* Current row counter (pass index)	;* DBG: DBG_CHUNK_BASE + offset cur_row_ctr_lbl
cur_pass_ctr	equ	4C5Eh			;* Current pass counter (outer blit loop)	;* DBG: DBG_CHUNK_BASE + offset cur_pass_ctr_lbl
render_fn_ptr	equ	4C60h			;* Pointer to active render sub-function	;* DBG: DBG_CHUNK_BASE + offset render_fn_ptr_lbl
saved_di	equ	4C62h			;* Saved destination offset (DI)
saved_es	equ	4C64h			;* Saved destination segment (ES)	;* DBG: DBG_CHUNK_BASE + offset saved_es_lbl
...
; ----------------------------------------------------------------------
src_word_a	equ	4C54h			;* Render source word A (plane 0)	;* DBG: DBG_CHUNK_BASE + offset src_word_a_lbl
src_word_b	equ	4C56h			;* Render source word B (plane 1)	;* DBG: DBG_CHUNK_BASE + offset src_word_b_lbl
src_word_c	equ	4C58h			;* Render source word C (plane 2)	;* DBG: DBG_CHUNK_BASE + offset src_word_c_lbl
src_word_d	equ	4C5Ah			;* Render source word D (plane 3)	;* DBG: DBG_CHUNK_BASE + offset src_word_d_lbl
render_mode_flag	equ	4C5Fh			;* Render mode: 0=overwrite, FF=mask-blend	;* DBG: DBG_CHUNK_BASE + offset render_mode_flag_lbl
