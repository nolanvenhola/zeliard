...
tga_mask_tbl_a		equ	32FCh	;* DBG: DBG_CHUNK_BASE + offset tga_mask_tbl_a_lbl
tga_mask_tbl_b		equ	3304h	;* DBG: DBG_CHUNK_BASE + offset tga_mask_tbl_b_lbl
sprite_src_tbl		equ	3640h	;* DBG: DBG_CHUNK_BASE + offset sprite_src_tbl_lbl
sprite_frame_tbl	equ	3642h	;* DBG: DBG_CHUNK_BASE + offset sprite_frame_tbl_lbl
move_seq_up	equ	3B04h			;* DBG: DBG_CHUNK_BASE + offset move_seq_up_lbl
move_seq_horiz		equ	3BC8h			;*
color_pair_tbl		equ	3BFBh	;* DBG: DBG_CHUNK_BASE + offset color_pair_tbl_lbl
color_pair_tbl_b	equ	3BFCh	;* DBG: DBG_CHUNK_BASE + offset color_pair_tbl_b_lbl
tga_palette_xlat	equ	4BD4h	;* DBG: DBG_CHUNK_BASE + offset tga_palette_xlat_lbl
tga_color_lut		equ	4BFDh	;* DBG: DBG_CHUNK_BASE + offset tga_color_lut_lbl
cur_col_ctr		equ	4C09h	;* DBG: DBG_CHUNK_BASE + offset cur_col_ctr_lbl
cur_row_ctr		equ	4C0Ah	;* DBG: DBG_CHUNK_BASE + offset cur_row_ctr_lbl
cur_pass_ctr		equ	4C0Bh	;* DBG: DBG_CHUNK_BASE + offset cur_pass_ctr_lbl
render_fn_ptr		equ	4C0Fh	;* DBG: DBG_CHUNK_BASE + offset render_fn_ptr_lbl
saved_di		equ	4C11h	;* DBG: DBG_CHUNK_BASE + offset saved_di_lbl
saved_es		equ	4C13h	;* DBG: DBG_CHUNK_BASE + offset saved_es_lbl
...
; ----------------------------------------------------------------------
src_word_a		equ	4BFFh	;* DBG: DBG_CHUNK_BASE + offset src_word_a_lbl
src_word_b		equ	4C01h	;* DBG: DBG_CHUNK_BASE + offset src_word_b_lbl
src_word_c		equ	4C03h	;* DBG: DBG_CHUNK_BASE + offset src_word_c_lbl
src_word_d		equ	4C05h	;* DBG: DBG_CHUNK_BASE + offset src_word_d_lbl
src_word_e		equ	4C07h	;* DBG: DBG_CHUNK_BASE + offset src_word_e_lbl
render_mode_flag	equ	4C0Ch	;* DBG: DBG_CHUNK_BASE + offset render_mode_flag_lbl
...
render_1plane_blit:
		...
		push	es
		push	di
		call	compute_tga_framebuf_offset
		mov	di,ax
		pop	si
		pop	ds
		pop	ax
		mov	word ptr cs:[render_fn_ptr],32CAh
		call	run_render_passes_tga
		pop	ds
		retn
...
run_render_passes_tga		proc	near
...
build_pixel_pair_tga		proc	near
		add	al,al
		sbb	ah,ah
		and	ah,0F0h
		add	al,al
...
copy_pixel_row_tga		endp

copy_to_di_tga		proc	near
		push	di
		push	cx
...
char_render_col_loop:
												...
												push	ds
												push	si
												call	compute_tile_vram_offset_tga
												pop	si
												pop	ds
												pop	bx
												inc	bh
...
compute_tile_vram_offset_tga		proc	near
		mov	ds,cs:[gvar_game_seg]
		mov	dx,cs
		add	dx,2000h
		mov	es,dx
...
compute_tile_vram_offset_tga		endp
...
lookup_palette_entry_tga		proc	near
		push	si
		dec	al
		xor	ah,ah			; Zero register
		add	ax,ax
...
lookup_palette_entry_tga		endp

extract_pixel_bits_tga		proc	near
...
blit_sprite_clipped_tga		proc	near
		push	bx
...
txt_row_wrap:
		mov	cx,2
		call	clear_status_buf_rows_tga
		pop	cx

txt_border_loop:
							push	cx
							call	write_status_pattern_4444_tga
...
init_status_buf_tga		endp

clear_status_buf_rows_tga		proc	near
...
clear_status_buf_rows_tga		endp

write_status_pattern_4444_tga		proc	near
		mov	word ptr es:[di-3],4444h
		mov	word ptr es:[di-1],4444h
		retn

write_status_pattern_4444_tga		endp
...
face_render_loop:
							...
							call	init_status_row_11_tga
							jmp	short face_row_done

face_row_narrow:
							call	init_status_row_28_tga
...
init_status_row_28_tga		proc	near
		mov	cx,28h
		mov	word ptr cs:[src_word_d],0
...
init_status_row_28_tga		endp

init_status_row_11_tga		proc	near
		mov	cx,0Bh
		mov	word ptr cs:[src_word_d],0
...
init_status_row_11_tga		endp

seed_status_pattern_tga		proc	near
		push	di
		mov	ax,0FC3Fh
		call	fill_status_byte_24x_tga
		add	di,36h
		mov	cx,5Bh
...
seed_status_pattern_tga		endp

fill_status_byte_24x_tga		proc	near
		stosb				; Store al to es:[di]
		mov	al,0FFh
		mov	cx,18h
		rep	stosb			; Rep when cx >0 Store al to es:[di]
...
face2_render_loop:
							...
							add	ax,39h
							add	ax,ax
							call	compute_vram_xy_offset_tga
							pop	ax
							push	ax
							add	ax,ax
							dec	ax
...
compute_vram_xy_offset_tga		proc	near
		push	ax
		mov	bl,al
		mov	al,2Fh			; '/'
		mul	bl			; ax = reg * al
...
compute_vram_xy_offset_tga		endp
...
copy_status_loop_tga		proc	near
		push	cx
		push	si
...
copy_status_loop_tga		endp

rotate_mask_word_tga		proc	near
...
compute_tga_framebuf_offset		proc	near
		add	bh,bh
		mov	dh,bl
		ror	dh,1			; Rotate
		ror	dh,1			; Rotate
...
compute_tga_framebuf_offset		endp
