...
dispatch_tbl_a		equ	3568h			;*  draw-cell dispatch table A	;* DBG: DBG_CHUNK_BASE + offset dispatch_tbl_a_lbl
dispatch_tbl_b		equ	35C4h			;*  draw-cell dispatch table B	;* DBG: DBG_CHUNK_BASE + offset dispatch_tbl_b_lbl
tile_gfx_data		equ	3811h			;*  tile graphics row data
status_buf	equ	DBG_CHUNK_BASE + offset status_buf_lbl		;*  time/status display buffer (7 bytes)
anim_frame_data		equ	39B4h			;*  animation frame data table (byte 1 of status_buf)
tile_render_disp	equ	3C04h			;*  tile type render dispatch table	;* DBG: DBG_CHUNK_BASE + offset tile_render_disp_lbl
color_lut		equ	3D0Bh			;*  color lookup table (bitplane index->CGA color)	;* DBG: DBG_CHUNK_BASE + offset color_lut_lbl
tile_col_x		equ	3D6Dh			;*  current tile column x position (word)	;* DBG: DBG_CHUNK_BASE + offset tile_col_x_lbl
tile_char_a		equ	3D70h			;*  tile char slot A (byte)	;* DBG: DBG_CHUNK_BASE + offset tile_char_a_lbl
tile_map_arr		equ	3D71h			;*  tile map working array
tile_char_b		equ	3D73h			;*  tile char slot B (byte, checked for 0FDh)	;* DBG: DBG_CHUNK_BASE + offset tile_char_b_lbl
tile_row_ctr		equ	3D76h			;*  tile row/scan counter (word)	;* DBG: DBG_CHUNK_BASE + offset tile_row_ctr_lbl
bitplane_0		equ	3D78h			;*  bitplane word 0 for tile color encode	;* DBG: DBG_CHUNK_BASE + offset bitplane_0_lbl
bitplane_1		equ	3D7Ah			;*  bitplane word 1 for tile color encode	;* DBG: DBG_CHUNK_BASE + offset bitplane_1_lbl
bitplane_2		equ	3D7Ch			;*  bitplane word 2 for tile color encode	;* DBG: DBG_CHUNK_BASE + offset bitplane_2_lbl
bitplane_3		equ	3D7Eh			;*  bitplane word 3 (transparent mask)	;* DBG: DBG_CHUNK_BASE + offset bitplane_3_lbl
far_ptr_tmp		equ	3D80h			;*  temporary far pointer (dword)	;* DBG: DBG_CHUNK_BASE + offset far_ptr_tmp_lbl
...
tile_cache		equ	3FD4h			;*  tile pixel cache table (indexed by tile id)	;* DBG: DBG_CHUNK_BASE + offset tile_cache_lbl
...
tile_col_idx		equ	3D6Fh			;*  current tile column counter (byte, 0..1Bh)	;* DBG: DBG_CHUNK_BASE + offset tile_col_idx_lbl
