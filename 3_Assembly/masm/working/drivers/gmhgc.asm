...
;                  hgc_dispatch_fn (mode-table jmp inside driver)
;    Called by:    zeliad.exe loader (when RESOURCE.CFG selects HGC mode);
...
dispatch_call:
		db	 50h			; push ax      (save fn#)
				; dual-use byte trick; target 0E11h)
		db	0E8h, 0C7h		; call calc_hgc_address  (E8 C7 0D: 0Dh = first byte of next 'or ax' = CALL high displacement
...
render_large_tilemap_a:				; called externally: render large tilemap A (cs:player_almas anim, di=2559h, cx=105h)
		...
		hgc_call_tile_large
				...
				; called externally: render large tilemap B (cs:gold_carried_x1/cs:gold_carried_x65536 anim, di=2558h, cx=106h)
render_large_tilemap_b:
		push	ds
...
fn_9:
				...
				; called externally: render large tilemap C (cs:selected_spell frame select via anim_lut, di=255Bh, cx=103h)
render_large_tilemap_c:
		push	ds
