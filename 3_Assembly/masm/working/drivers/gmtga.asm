...
fn_9:
		retn
				...
				; dispatch fn 2 (CS:2503) ?-- render time display B: reads cs:[gold_carried_x1]/cs:[gold_carried_x65536] timer, tilemap_src=
fn2_render_time_b:
		push	ds
...
fn_10:
		...
		retn
				...
				; dispatch fn 3 (CS:2523) ?-- render time display C: indexed by cs:[selected_spell] frame, color LUT at CS:[0ABh], tilemap_s
fn3_render_time_c:
		push	ds
...
fn_12:
				...
				; dispatch fn 4 (CS:254C) ?-- sprite visibility check: early-out if cs:[shield]==0, else render time display D (tilemap_src=
fn4_sprite_check:
...
decode_bitplane_tile		endp
				...
				; dispatch fn 5 (CS:26B0) ?-- render large sprite: AL=frame, anim_ptr_0 base, 18 rows ?? 15 bytes/row, 3 bitplanes
fn5_sprite_large:
		push	ds
...
render_text_char_alt		endp
				...
				; dispatch fn 9 (CS:2A68) ?-- TGA?->TGA sprite copy: BH/BL coords, CH=height, row-by-row rep movsw from src+2000h
fn9_sprite_copy:
		push	ds
		add	bh,bh
		add	bh,bh
...
sprite_save_wrap:
		...
		retn
				...
				; dispatch fn 11 (CS:2B0F) ?-- restore sprite BG: CS+3000h scratch ?-> TGA framebuf, DI=src addr, AL=row AH=col, CH=height
fn11_sprite_restore:
		push	ds
		mov	si,di
		add	si,0
...
sprite_restore_wrap:
		...
		retn
				...
				; dispatch fn 12 (CS:2B65) ?-- render string: BX=char src ptr, CL=bit idx, SI=string (0xFF=end, 0x0D=newline, hi-bit=color)
fn12_string_render:
