...
dispatch_tbl	equ	317Eh			;* function dispatch table (word array)	;* DBG: DBG_CHUNK_BASE + offset dispatch_tbl_lbl
pattern_ptr_tbl	equ	3844h			;* pattern pointer table	;* DBG: DBG_CHUNK_BASE + offset pattern_ptr_tbl_lbl
sprite_tmp_buf	equ	DBG_CHUNK_BASE + offset gf_hgc_phase_idx_tbl		;* sprite temporary pixel buffer
color_map_tbl	equ	46D0h			;* color map table (word array)	;* DBG: DBG_CHUNK_BASE + offset color_map_tbl_lbl
phase_offset_tbl equ	4982h			;* phase/shift offset table
bg_tile_src	equ	4C38h			;* background tile source pointer
copy_fn_tbl	equ	4EFFh			;* HGC copy function pointer table (word array)	;* DBG: DBG_CHUNK_BASE + offset copy_fn_tbl_lbl
internal_tbl_68	equ	DBG_CHUNK_BASE + offset cycle_tbl_a		;* internal 64-byte table (copied to hgc_plane_buf_a)
hero_gfx_tbl	equ	4F86h			;* hero graphics data table	;* DBG: DBG_CHUNK_BASE + offset hero_gfx_tbl_lbl
cur_color_pair	equ	4FE2h			;* current color pair word (set from palette byte)	;* DBG: DBG_CHUNK_BASE + offset cur_color_pair_lbl
...
scroll_gfx_ptr	equ	4FF1h			;* scroll graphics data pointer (word)	;* DBG: DBG_CHUNK_BASE + offset scroll_gfx_ptr_lbl
scroll_delta	equ	4FF3h			;* scroll delta (word: col byte + row byte)	;* DBG: DBG_CHUNK_BASE + offset scroll_delta_lbl
shift_count	equ	4FF5h			;* shift count byte	;* DBG: DBG_CHUNK_BASE + offset shift_count_lbl
sprite_row_buf	equ	4FF9h			;* sprite row intermediate buffer (word)
sprite_pos	equ	500Dh			;* sprite position word (col/row packed)
sprite_cache_tbl equ	5016h			;* sprite HGC cache table (word array, indexed by slot*2)	;* DBG: DBG_CHUNK_BASE + offset sprite_cache_tbl_lbl
...
; ----------------------------------------------------------------------
scroll_state	equ	4FE8h			;* scroll state word	;* DBG: DBG_CHUNK_BASE + offset scroll_state_lbl
row_counter	equ	4FEAh			;* row countdown (0x12 rows per frame)	;* DBG: DBG_CHUNK_BASE + offset row_counter_lbl
col_idx		equ	4FEBh			;* current column index (byte)	;* DBG: DBG_CHUNK_BASE + offset col_idx_lbl
row_idx		equ	4FECh			;* current row index (byte)	;* DBG: DBG_CHUNK_BASE + offset row_idx_lbl
palette_byte	equ	4FEDh			;* palette byte for current sprite	;* DBG: DBG_CHUNK_BASE + offset palette_byte_lbl
hgc_mask_state	equ	4FF8h			;* HGC mask state byte (writer fill mask)	;* DBG: DBG_CHUNK_BASE + offset hgc_mask_state_lbl
sprite_state_a	equ	5009h			;* sprite slot state byte A (0xFF=empty, 0xFC=hidden)	;* DBG: DBG_CHUNK_BASE + offset sprite_state_a_lbl
sprite_state_b	equ	500Ah			;* sprite slot state byte B	;* DBG: DBG_CHUNK_BASE + offset sprite_state_b_lbl

; ----------------------------------------------------------------------
; Section 7: Constants
; ----------------------------------------------------------------------
anim_frame_tbl	equ	37A2h			;* animation frame offset table
anim_phase	equ	4FF7h			;* animation pass counter (0-7, decremented)	;* DBG: DBG_CHUNK_BASE + offset anim_phase_lbl
...
sprite_scan_loop:
				push	cx
				SPRITE_SLOT_SCAN_STEP scan_s1
				SPRITE_SLOT_SCAN_STEP scan_s2
				SPRITE_SLOT_SCAN_STEP scan_s3
				SPRITE_SLOT_SCAN_STEP scan_s4
				pop	cx
				loop	sprite_scan_loop	; Loop if cx > 0

		SPRITE_SLOT_SCAN_STEP scan_tail_a
		SPRITE_SLOT_SCAN_STEP scan_tail_b
		test	byte ptr [si],80h
...
col_scan_loop:
						push	cx
						SPRITE_STATE_SCAN_STEP col_s1
						SPRITE_STATE_SCAN_STEP col_s2
						SPRITE_STATE_SCAN_STEP col_s3
						SPRITE_STATE_SCAN_STEP col_s4
						pop	cx
						loop	col_scan_loop		; Loop if cx > 0

				SPRITE_STATE_SCAN_STEP col_tail_a
				SPRITE_STATE_SCAN_STEP col_tail_b
				SPRITE_STATE_SCAN_STEP col_tail_c
				lodsb				; String [si] to al
				inc	di
				or	al,al			; Zero ?
...
loc_62:
		add	dx,40B4h
		cmp	dx,6000h
		jb	check_row_counter_eq_1			; Jump if below
		add	dx,0A05Ah

check_row_counter_eq_1:
...
loc_66:
		...
		add	dx,40B0h
		cmp	dx,6000h
		jb	call_sprite_pair_blit			; Jump if below
		add	dx,0A05Ah

call_sprite_pair_blit:
		call	sprite_pair_blit
...
loc_71:
		add	dx,40B4h
		cmp	dx,6000h
		jb	check_row_counter_eq_1_72			; Jump if below
		add	dx,0A05Ah

check_row_counter_eq_1_72:
		cmp	byte ptr ds:[row_counter],1
...
test_flag_hero_state:
		...
		jz	call_get_step_direction			; Jump if zero
		inc	cl
		...
		add	si,ax
		add	si,62C7h
		jmp	short test_flag_shield_124
...
loc_133:
		...
		or	al,ds:[flag_riding]
		jz	test_flag_hero_state_135			; Jump if zero
		call	get_step_direction
		or	al,al			; Zero ?
...
loc_169:
		...
		or	bl,bl			; Zero ?
		jz	test_flag_shield_175			; Jump if zero
...
loc_171:
		...
		jmp	short test_flag_shield_175
...
;--------------------------------------------------------------------------
; Sprite/animation lookup tables (chunk_04 file offsets 0x1804..0x18C4;
; 193 bytes).  Earlier "Character encoding / font lookup table" + "tile
; pair lookup" interleaved blocks here were Sourcer-mis-decoded from data
; — they emitted ~67 bytes of bogus output instead of the original 193.
; Restored verbatim from chunk_04.bin extracted bytes.
;--------------------------------------------------------------------------
		db	000h, 000h, 000h, 000h, 000h, 000h, 000h, 000h, 000h, 069h, 071h, 073h	; 0x1804
		db	074h, 01Fh, 020h, 02Ch, 027h, 01Fh, 020h, 017h, 018h, 038h, 03Ah, 03Fh	; 0x1810
		db	042h, 045h, 000h, 000h, 000h, 000h, 000h, 000h, 000h, 000h, 000h, 000h	; 0x181C
		db	000h, 000h, 06Dh, 075h, 077h, 079h, 06Fh, 02Bh, 026h, 029h, 026h, 01Ah	; 0x1828
		db	034h, 039h, 03Bh, 040h, 041h, 000h, 000h, 000h, 000h, 000h, 000h, 000h	; 0x1834
		db	000h, 000h, 000h, 000h, 000h, 000h, 000h, 076h, 078h, 07Ah, 07Bh, 031h	; 0x1840
		db	032h, 02Fh, 02Dh, 033h, 036h, 037h, 03Ch, 000h, 000h, 000h, 000h, 000h	; 0x184C
		db	000h, 000h, 000h, 000h, 000h, 000h, 000h, 000h, 000h, 000h, 000h, 000h	; 0x1858
		db	000h, 000h, 06Dh, 071h, 070h, 072h, 070h, 000h, 000h, 000h, 000h, 000h	; 0x1864
		db	000h, 000h, 000h, 000h, 000h, 000h, 000h, 000h, 000h, 000h, 000h, 000h	; 0x1870
		db	000h, 000h, 000h, 000h, 000h, 000h, 000h, 000h, 000h, 000h, 000h, 000h	; 0x187C
		db	000h, 000h, 000h, 000h, 000h, 000h, 000h, 000h, 000h, 000h, 000h, 000h	; 0x1888
		db	000h, 000h, 000h, 000h, 000h, 000h, 000h, 000h, 000h, 000h, 000h, 000h	; 0x1894
		db	000h, 000h, 000h, 000h, 000h, 000h, 000h, 000h, 000h, 000h, 000h, 000h	; 0x18A0
		db	000h, 000h, 000h, 000h, 000h, 000h, 000h, 000h, 000h, 000h, 000h, 000h	; 0x18AC
		db	000h, 000h, 000h, 000h, 000h, 000h, 000h, 001h, 002h, 001h, 002h, 001h	; 0x18B8
		db	002h	; 0x18C4
; Tile pair entries continuation (chunk_04 0x18C5..0x1912)
		db	001h, 002h, 001h, 002h, 001h, 002h, 001h, 002h, 001h, 002h, 001h, 002h	; 0x18C5
		db	001h, 002h, 001h, 002h, 001h, 002h, 001h, 002h, 001h, 002h, 003h, 004h	; 0x18D1
		db	003h, 004h, 003h, 004h, 003h, 004h, 003h, 004h, 003h, 004h, 003h, 004h	; 0x18DD
		db	003h, 004h, 003h, 004h, 003h, 004h, 003h, 004h, 003h, 004h, 003h, 004h	; 0x18E9
		db	003h, 004h, 005h, 006h, 005h, 006h, 005h, 006h, 005h, 006h, 005h, 006h	; 0x18F5
		db	005h, 006h, 005h, 006h, 005h, 006h, 005h, 006h, 005h, 006h, 005h, 006h	; 0x1901
		db	006h, 005h, 005h, 006h, 005h, 006h	; 0x190D
