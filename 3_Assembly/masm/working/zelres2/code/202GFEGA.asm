...
;
;  Key subsystems:
;    run_gfega_main          - main per-frame entry: scan sprite slots, render rows
...
;    render_frame_rows    - per-frame row dispatcher: scrolling, BG save/restore
...
;                  internal CS dispatchers (render_frame_rows, anim_refresh_all,
...
; ----------------------------------------------------------------------
dispatch_tbl             equ     3170h	;* DBG: DBG_CHUNK_BASE + offset dispatch_tbl_lbl
...
pattern_ptr_tbl	equ	3929h			;* pattern pointer table	;* DBG: DBG_CHUNK_BASE + offset pattern_ptr_tbl_lbl
sprite_tmp_buf	equ	3E80h			;* sprite temporary pixel buffer
color_map_tbl	equ	4199h			;* EGA color map table	;* DBG: DBG_CHUNK_BASE + offset color_map_tbl_lbl
phase_offset_tbl equ	43C8h			;* phase/shift offset table
bg_tile_src	equ	DBG_CHUNK_BASE + offset ui_tile_idx_tbl		;* background tile source pointer
copy_fn_tbl	equ	485Ch			;* VGA copy function pointer table (word array)	;* DBG: DBG_CHUNK_BASE + offset copy_fn_tbl_lbl
hero_gfx_tbl	equ	4B88h			;* hero graphics data table	;* DBG: DBG_CHUNK_BASE + offset hero_gfx_tbl_lbl
color_pair_tbl	equ	505Ah			;* EGA color pair lookup table (word array)	;* DBG: DBG_CHUNK_BASE + offset color_pair_tbl_lbl
cur_color_pair	equ	5067h			;* current EGA color pair word (set from table)	;* DBG: DBG_CHUNK_BASE + offset cur_color_pair_lbl
...
scroll_gfx_ptr	equ	5074h			;* scroll graphics data pointer (word)	;* DBG: DBG_CHUNK_BASE + offset scroll_gfx_ptr_lbl
scroll_delta	equ	5076h			;* scroll delta (word: col byte + row byte)	;* DBG: DBG_CHUNK_BASE + offset scroll_delta_lbl
shift_count	equ	5079h			;* EGA shift count byte	;* DBG: DBG_CHUNK_BASE + offset shift_count_lbl
sprite_row_buf	equ	507Ah			;* sprite row intermediate buffer (word)
sprite_pos	equ	508Eh			;* sprite position word (col/row packed)	;* DBG: DBG_CHUNK_BASE + offset sprite_pos_lbl
sprite_cache_tbl equ	5097h			;* sprite EGA cache table (word array, indexed by slot*2)	;* DBG: DBG_CHUNK_BASE + offset sprite_cache_tbl_lbl
...
; ----------------------------------------------------------------------
row_counter	equ	506Dh			;* row countdown (0x12 rows per frame)	;* DBG: DBG_CHUNK_BASE + offset row_counter_lbl
col_idx		equ	506Eh			;* current column index (byte)	;* DBG: DBG_CHUNK_BASE + offset col_idx_lbl
row_idx		equ	506Fh			;* current row index (byte)	;* DBG: DBG_CHUNK_BASE + offset row_idx_lbl
palette_byte	equ	5070h			;* EGA color/palette byte for current sprite	;* DBG: DBG_CHUNK_BASE + offset palette_byte_lbl
bitmask_byte	equ	5071h			;* EGA bit mask byte for blitter	;* DBG: DBG_CHUNK_BASE + offset bitmask_byte_lbl
sprite_state_a	equ	508Ah			;* sprite slot state byte A (0xFF=empty, 0xFC=hidden)	;* DBG: DBG_CHUNK_BASE + offset sprite_state_a_lbl
sprite_state_b	equ	508Bh			;* sprite slot state byte B	;* DBG: DBG_CHUNK_BASE + offset sprite_state_b_lbl
...
anim_phase	equ	5078h			;* animation pass counter (0-7, decremented)	;* DBG: DBG_CHUNK_BASE + offset anim_phase_lbl
...
; run_gfega_main inline init block (0x000B-0x005D):
...
loc_52:
		...
		mov	al,bh
		or	al,al			; Zero ?
		jns	call_sprite_cell_render_53			; Jump if not sign
		call	sprite_get_value

call_sprite_cell_render_53:
		call	sprite_cell_render
...
loc_59:
		...
		mov	al,bh
		or	al,al			; Zero ?
		jns	call_sprite_cell_render_60			; Jump if not sign
		call	sprite_get_value

call_sprite_cell_render_60:
		call	sprite_cell_render
...
test_flag_hero_state:
		...
		add	si,ax
		add	si,62C7h
		jmp	short test_flag_shield_104
...
loc_113:
		...
		or	al,ds:[flag_riding]
		jz	test_flag_hero_state_115			; Jump if zero
		call	hero_tier_get
		or	al,al			; Zero ?
...
loc_129:
		...
		pop	ax
		or	al,al			; Zero ?
		jz	call_ega_clear_16bytes			; Jump if zero
		mov	cl,sprite_data_stride
		mul	cl			; ax = reg * al
		add	ax,sprite_src_base
		mov	si,ax
...
loc_132:
		...
		pop	ax
		or	al,al			; Zero ?
		jz	call_ega_sprite_render_solid			; Jump if zero
		mov	cl,al
		call	ega_sprite_blit_ex
		pop	di
		pop	si
...
; Companion to wrap_scroll_si_low: wraps SI upward (if SI < 0xE000, advance by 0x900).
