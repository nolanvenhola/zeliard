...
dispatch_tbl	equ	3176h			;* function dispatch table (word array)	;* DBG: DBG_CHUNK_BASE + offset dispatch_tbl_lbl
pattern_ptr_tbl	equ	389Bh			;* pattern pointer table	;* DBG: DBG_CHUNK_BASE + offset pattern_ptr_tbl_lbl
init_tbl_59	equ	3963h			;* init table at 3963h (referenced in start header)
init_tbl_60	equ	DBG_CHUNK_BASE + offset init_tbl_60_lbl		;* init table at 3F38h (referenced in start header)
color_map_tbl	equ	4086h			;* color map/pair word table	;* DBG: DBG_CHUNK_BASE + offset color_map_tbl_lbl
phase_offset_tbl equ	4588h			;* phase/shift offset table
bg_tile_src	equ	46D4h			;* background tile source pointer	;* DBG: DBG_CHUNK_BASE + offset bg_tile_src_lbl
copy_fn_tbl	equ	4A25h			;* copy function pointer table (word array)	;* DBG: DBG_CHUNK_BASE + offset copy_fn_tbl_lbl
hero_gfx_tbl	equ	4F8Ch			;* hero graphics data table	;* DBG: DBG_CHUNK_BASE + offset hero_gfx_tbl_lbl
cur_color_pair	equ	4FE9h			;* current color pair word (set from table)	;* DBG: DBG_CHUNK_BASE + offset cur_color_pair_lbl
vga_row_ptr	equ	4FEBh			;* current VGA row byte offset (word)
scroll_vga_ofs	equ	4FEDh			;* scroll destination VGA byte offset (word)
rle_tmp_a	equ	4FEFh			;* RLE temp word A (cs: scratch)	;* DBG: DBG_CHUNK_BASE + offset rle_tmp_a_lbl
rle_tmp_b	equ	4FF5h			;* RLE temp word B (cs: scratch)	;* DBG: DBG_CHUNK_BASE + offset rle_tmp_b_lbl
scroll_src_ofs	equ	4FF7h			;* scroll source VGA byte offset (word)
scroll_gfx_ptr	equ	4FF9h			;* scroll graphics data pointer (word)	;* DBG: DBG_CHUNK_BASE + offset scroll_gfx_ptr_lbl
scroll_delta	equ	4FFBh			;* scroll delta (word: col byte + row byte)	;* DBG: DBG_CHUNK_BASE + offset scroll_delta_lbl
rle_mask	equ	4FFDh			;* RLE mask word (cs: scratch)	;* DBG: DBG_CHUNK_BASE + offset rle_mask_lbl
sprite_row_buf	equ	5000h			;* sprite row intermediate buffer base
sprite_pos	equ	5014h			;* sprite position word (col/row packed)
sprite_cache_tbl equ	501Dh			;* sprite cache table (word array, indexed by slot*2)	;* DBG: DBG_CHUNK_BASE + offset sprite_cache_tbl_lbl
...
; ----------------------------------------------------------------------
row_counter	equ	4FF1h			;* row countdown (0x12 rows per frame)	;* DBG: DBG_CHUNK_BASE + offset row_counter_lbl
col_idx		equ	4FF2h			;* current column index (byte)	;* DBG: DBG_CHUNK_BASE + offset col_idx_lbl
row_idx		equ	4FF3h			;* current row index (byte)	;* DBG: DBG_CHUNK_BASE + offset row_idx_lbl
palette_byte	equ	4FF4h			;* palette byte for current sprite	;* DBG: DBG_CHUNK_BASE + offset palette_byte_lbl
sprite_state_a	equ	5010h			;* sprite slot state byte A (0xFF=empty, 0xFC=hidden)	;* DBG: DBG_CHUNK_BASE + offset sprite_state_a_lbl
sprite_state_b	equ	5011h			;* sprite slot state byte B	;* DBG: DBG_CHUNK_BASE + offset sprite_state_b_lbl

; ----------------------------------------------------------------------
; Section 7: Constants
; ----------------------------------------------------------------------
anim_frame_tbl	equ	3893h			;* animation frame offset table
anim_phase	equ	4FFFh			;* animation pass counter (0-7, decremented)	;* DBG: DBG_CHUNK_BASE + offset anim_phase_lbl
...
init_scan_loop:
		...
		SPRITE_SLOT_SCAN_STEP loc_8
		SPRITE_SLOT_SCAN_STEP loc_9
		test	byte ptr [si],80h
...
col_scan_loop:
				...
				SPRITE_STATE_SCAN_STEP loc_20
				SPRITE_STATE_SCAN_STEP loc_21
				lodsb				; String [si] to al
				inc	di
				or	al,al			; Zero ?
...
loc_57:
		...
		mov	al,bh
		or	al,al			; Zero ?
		jns	call_sprite_cell_render_58			; Jump if not sign
		call	sprite_get_value

call_sprite_cell_render_58:
		call	sprite_cell_render
...
loc_62:
		...
		push	dx
		or	al,al			; Zero ?
		jns	call_sprite_cell_render_63			; Jump if not sign
		call	sprite_get_value

call_sprite_cell_render_63:
		call	sprite_cell_render
		pop	dx
		pop	si
		pop	bx
...
loc_64:
		...
		mov	al,bh
		or	al,al			; Zero ?
		jns	call_sprite_cell_render_65			; Jump if not sign
		call	sprite_get_value

call_sprite_cell_render_65:
		call	sprite_cell_render
...
test_flag_hero_state:
		...
		jz	call_shield_state_get			; Jump if zero
		inc	cl
		...
		add	si,ax
		add	si,62C7h
		jmp	short test_flag_shield_115
...
loc_124:
		...
		or	al,ds:[flag_riding]
		jz	test_flag_hero_state_126			; Jump if zero
		call	shield_state_get
		or	al,al			; Zero ?
...
loc_140:
		...
		pop	ax
		or	al,al			; Zero ?
		jz	call_mca_sprite_clear_cell			; Jump if zero
		dec	al
		mov	cl,30h			; '0'
		mul	cl			; ax = reg * al
		add	ax,8030h
...
loc_143:
		...
		pop	ax
		or	al,al			; Zero ?
		jz	call_mca_plane_copy_16rows			; Jump if zero
		mov	cl,al
		call	mca_sprite_blit_ex
		pop	di
		pop	si
...
check_gvar_frame_timer_eq_10:
				...
				jnz	set_vga_row_ptr_11B0			; Jump if not zero
		retn
...
mca_color_fade_init:
		...
		mov	ah,byte ptr ds:[fight_player_col]	; Y coord (game_seg global)
		add	ah,ah			; Y * 2
		add	ah,ah			; Y * 4
...
;--------------------------------------------------------------------------
; Sprite/animation lookup tables (chunk_06 file offsets 0x1828..0x18E8;
...
; Restored verbatim from chunk_06.bin extracted bytes.
;--------------------------------------------------------------------------
		db	000h, 000h, 000h, 000h, 000h, 000h, 000h, 000h, 000h, 069h, 071h, 073h	; 0x1828
		db	074h, 01Fh, 020h, 02Ch, 027h, 01Fh, 020h, 017h, 018h, 038h, 03Ah, 03Fh	; 0x1834
		db	042h, 045h, 000h, 000h, 000h, 000h, 000h, 000h, 000h, 000h, 000h, 000h	; 0x1840
		db	000h, 000h, 06Dh, 075h, 077h, 079h, 06Fh, 02Bh, 026h, 029h, 026h, 01Ah	; 0x184C
		db	034h, 039h, 03Bh, 040h, 041h, 000h, 000h, 000h, 000h, 000h, 000h, 000h	; 0x1858
		db	000h, 000h, 000h, 000h, 000h, 000h, 000h, 076h, 078h, 07Ah, 07Bh, 031h	; 0x1864
		db	032h, 02Fh, 02Dh, 033h, 036h, 037h, 03Ch, 000h, 000h, 000h, 000h, 000h	; 0x1870
		db	000h, 000h, 000h, 000h, 000h, 000h, 000h, 000h, 000h, 000h, 000h, 000h	; 0x187C
		db	000h, 000h, 06Dh, 071h, 070h, 072h, 070h, 000h, 000h, 000h, 000h, 000h	; 0x1888
		...
		db	000h, 000h, 000h, 000h, 000h, 000h, 000h, 000h, 000h, 000h, 000h, 000h	; 0x18B8
		db	000h, 000h, 000h, 000h, 000h, 000h, 000h, 000h, 000h, 000h, 000h, 000h	; 0x18C4
		db	000h, 000h, 000h, 000h, 000h, 000h, 000h, 000h, 000h, 000h, 000h, 000h	; 0x18D0
		db	000h, 000h, 000h, 000h, 000h, 000h, 000h, 001h, 002h, 001h, 002h, 001h	; 0x18DC
		db	002h	; 0x18E8
; Tile pair entries continuation (chunk_06 0x18E9..0x1936)
		db	001h, 002h, 001h, 002h, 001h, 002h, 001h, 002h, 001h, 002h, 001h, 002h	; 0x18E9
		db	001h, 002h, 001h, 002h, 001h, 002h, 001h, 002h, 001h, 002h, 003h, 004h	; 0x18F5
		db	003h, 004h, 003h, 004h, 003h, 004h, 003h, 004h, 003h, 004h, 003h, 004h	; 0x1901
		db	003h, 004h, 003h, 004h, 003h, 004h, 003h, 004h, 003h, 004h, 003h, 004h	; 0x190D
		db	003h, 004h, 005h, 006h, 005h, 006h, 005h, 006h, 005h, 006h, 005h, 006h	; 0x1919
		db	005h, 006h, 005h, 006h, 005h, 006h, 005h, 006h, 005h, 006h, 005h, 006h	; 0x1925
		db	006h, 005h, 005h, 006h, 005h, 006h	; 0x1931
