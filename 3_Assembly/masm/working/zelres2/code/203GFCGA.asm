...
pattern_ptr_tbl	equ	397Dh			;* pattern pointer table	;* DBG: DBG_CHUNK_BASE + offset pattern_ptr_tbl_lbl
sprite_tmp_buf	equ	DBG_CHUNK_BASE + offset gf_cga_phase_idx_tbl		;* sprite temporary pixel buffer
color_map_tbl	equ	4777h			;* CGA color map table	;* DBG: DBG_CHUNK_BASE + offset color_map_tbl_lbl
phase_offset_tbl equ	47BEh			;* phase/shift offset table
bg_tile_src	equ	4A3Ah			;* background tile source pointer	;* DBG: DBG_CHUNK_BASE + offset bg_tile_src_lbl
copy_fn_tbl	equ	4F88h			;* CGA copy function pointer table (word array)	;* DBG: DBG_CHUNK_BASE + offset copy_fn_tbl_lbl
hero_gfx_tbl	equ	DBG_CHUNK_BASE + offset pattern_tail_tbl		;* hero graphics data table
color_pair_tbl	equ	500Fh			;* CGA color pair lookup table (word array)	;* DBG: DBG_CHUNK_BASE + offset color_pair_tbl_lbl
cur_color_pair	equ	506Bh			;* current CGA color pair word (set from table)	;* DBG: DBG_CHUNK_BASE + offset cur_color_pair_lbl
...
scroll_gfx_ptr	equ	507Ah			;* scroll graphics data pointer (word)	;* DBG: DBG_CHUNK_BASE + offset scroll_gfx_ptr_lbl
scroll_delta	equ	507Ch			;* scroll delta (word: col byte + row byte)	;* DBG: DBG_CHUNK_BASE + offset scroll_delta_lbl
sprite_row_buf	equ	507Eh			;* sprite row intermediate buffer (word)	;* DBG: DBG_CHUNK_BASE + offset sprite_row_buf_lbl
shift_count	equ	5081h			;* CGA shift count byte	;* DBG: DBG_CHUNK_BASE + offset shift_count_lbl
sprite_work_buf	equ	5082h			;* sprite work buffer (word array)
sprite_pos	equ	5096h			;* sprite position word (col/row packed)
sprite_cache_tbl equ	509Fh			;* sprite CGA cache table (word array)	;* DBG: DBG_CHUNK_BASE + offset sprite_cache_tbl_lbl
...
; ----------------------------------------------------------------------
row_counter	equ	5073h			;* row countdown (0x12 rows per frame)	;* DBG: DBG_CHUNK_BASE + offset row_counter_lbl
col_idx		equ	5074h			;* current column index (byte)	;* DBG: DBG_CHUNK_BASE + offset col_idx_lbl
row_idx		equ	5075h			;* current row index (byte)	;* DBG: DBG_CHUNK_BASE + offset row_idx_lbl
palette_byte	equ	5076h			;* CGA color/palette byte for current sprite	;* DBG: DBG_CHUNK_BASE + offset palette_byte_lbl
bitmask_byte	equ	5077h			;* CGA bit mask byte for blitter	;* DBG: DBG_CHUNK_BASE + offset bitmask_byte_lbl
sprite_state_a	equ	5092h			;* sprite slot state byte A (0xFF=empty, 0xFC=hidden)	;* DBG: DBG_CHUNK_BASE + offset sprite_state_a_lbl
sprite_state_b	equ	5093h			;* sprite slot state byte B	;* DBG: DBG_CHUNK_BASE + offset sprite_state_b_lbl

; ----------------------------------------------------------------------
; Section 7: Constants
; ----------------------------------------------------------------------
anim_frame_tbl	equ	38D1h			;* animation frame offset table
anim_phase	equ	5080h			;* animation pass counter (decremented)	;* DBG: DBG_CHUNK_BASE + offset anim_phase_lbl
...
loc_75:
		...
		mov	al,bh
		or	al,al			; Zero ?
		jns	call_sprite_cell_render_76			; Jump if not sign
		call	sprite_get_value

call_sprite_cell_render_76:
		call	sprite_cell_render
...
sprite_neg_handler:
		...
		push	dx
		or	al,al			; Zero ?
		jns	call_sprite_cell_render_81			; Jump if not sign
		call	sprite_get_value

call_sprite_cell_render_81:
		call	sprite_cell_render
		pop	dx
		pop	si
		pop	bx
...
loc_82:
		...
		mov	al,bh
		or	al,al			; Zero ?
		jns	call_sprite_cell_render_83			; Jump if not sign
		call	sprite_get_value

call_sprite_cell_render_83:
		call	sprite_cell_render
...
test_flag_hero_state:
		...
		add	si,ax
		add	si,62C7h
		jmp	short test_flag_shield_132
...
loc_141:
		...
		or	al,ds:[flag_riding]
		jz	test_flag_hero_state_143			; Jump if zero
		call	hero_tier_get
		or	al,al			; Zero ?
...
loc_157:
		...
		pop	ax
		or	al,al			; Zero ?
		jz	call_sprite_clear_8words			; Jump if zero
		dec	al
		mov	cl,10h
		mul	cl			; ax = reg * al
		add	ax,8030h
...
loc_160:
		...
		pop	ax
		or	al,al			; Zero ?
		jz	call_cga_sprite_render_solid			; Jump if zero
		mov	cl,al
		call	cga_sprite_blit_ex
		pop	di
		pop	si
