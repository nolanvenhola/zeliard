...
def parse_lst(lst_text):
    ...
    seq = 0  # sequential counter for MASM LST (no line numbers)
    for line in lst_text.split('\n'):
        seq += 1
        # proc/label declarations → build offset→name map
        # TASM format: "   lnum   XXXX   name   proc/label"
        ...
        if m:
            ...
            continue
        # MASM format: " XXXX   name   proc" (no byte dump for proc declarations)
        m = re.match(r'^ ([0-9A-F]{4})\s+(\w+)\s+(proc|label)\b', line, re.I)
        if m:
            off  = int(m.group(1), 16)
            name = m.group(2)
            if off not in offset_to_label:
                offset_to_label[off] = name
            offset_table.append((seq, m.group(2), m.group(3).lower()))
            continue

        # Also capture plain labels (TASM only — MASM labels use different format)
        ...
        # Try TASM format first, then MASM format
        ...
        if m2:
            ...
            # MASM format: " OOOO  BB BB...  instruction"
            m2 = re.match(r'^ ([0-9A-F]{4})\s+((?:[0-9A-F]{2}\s+)+)(.*)', line)
            if not m2:
                continue
            src_lnum  = seq
            off       = int(m2.group(1), 16)
            hex_bytes = [int(x, 16) for x in m2.group(2).split()]
