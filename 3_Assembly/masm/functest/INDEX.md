...
| proc_equivalence | [test_non_mcga_display_inventory_oracle.py](proc_equivalence/test_non_mcga_display_inventory_oracle.py) | Exact release hashes and positive, unique procedure bounds for all 377 EGA/CGA/HGC/TGA GD/GT/GF overlay procedures plus their four base graphics drivers |
| proc_equivalence | [test_ega_display_procedure_oracle.py](proc_equivalence/test_ega_display_procedure_oracle.py) | Exact release execution of all 104 EGA GD/GT/GF/base-driver procedures across empty, normal, and edge-shaped states; fingerprints control flow, registers/flags, non-stack memory/framebuffer writes, interrupts, and EGA port traffic |
| proc_equivalence | [test_cga_display_procedure_oracle.py](proc_equivalence/test_cga_display_procedure_oracle.py) | Exact release execution of all 107 CGA GD/GT/GF/base-driver procedures across three machine states; fingerprints control flow, registers/flags, non-stack memory/B800 framebuffer writes, interrupts, and port traffic |
| proc_equivalence | [test_hgc_display_procedure_oracle.py](proc_equivalence/test_hgc_display_procedure_oracle.py) | Exact release execution of all 124 Hercules GD/GT/GF/base-driver procedures across three machine states; fingerprints control flow, registers/flags, non-stack memory/B000 framebuffer writes, interrupts, and port traffic |
| proc_equivalence | [test_tandy_display_procedure_oracle.py](proc_equivalence/test_tandy_display_procedure_oracle.py) | Exact release execution of all 119 Tandy GD/GT/GF/base-driver procedures across three machine states; fingerprints control flow, registers/flags, non-stack memory/B800 framebuffer writes, interrupts, and port traffic |
| proc_equivalence | [test_stick_remaining_procedure_oracle.py](proc_equivalence/test_stick_remaining_procedure_oracle.py) | Exact release execution of the remaining 11 stick driver procedures: joystick edges/calibration/dead zones, DOS file I/O and retry behavior, and decompressor dispatch/table helpers |
...
| proc_equivalence | [test_sword_wise_man_oracle.py](proc_equivalence/test_sword_wise_man_oracle.py) | Release `212ARMRP.bin`/`201SELCT.bin`/`200FIGHT.bin` Wise Man's Sword contract - armory replacement semantics, exact item/HUD text, attack audio edge, sword-tier lookup, native base power 2, experience/Sabre Oil modifiers, and attack-state doubling |
| proc_equivalence | [test_sword_spirit_oracle.py](proc_equivalence/test_sword_spirit_oracle.py) | Release `212ARMRP.bin`/`201SELCT.bin`/`200FIGHT.bin` Spirit Sword contract - armory replacement semantics, exact item/HUD text, attack audio edge, sword-tier lookup, native base power 4, experience/Sabre Oil modifiers, and attack-state doubling |
| proc_equivalence | [test_sword_knight_oracle.py](proc_equivalence/test_sword_knight_oracle.py) | Release `MP51`/`200FIGHT.bin`/`212ARMRP.bin`/`201SELCT.bin` Glory-Crest and Knight's Sword contract - Cementar object 55 at byte 24h/mask 80h, acquisition message/write to 9Bh, Tumba one-time gates, Yes/No exchange text, atomic crest consumption/equip/stock mutation, tier-4 asset load, exact HUD text, attack audio edge, and native base power 8 |
| proc_equivalence | [test_sword_illumination_oracle.py](proc_equivalence/test_sword_illumination_oracle.py) | Release `212ARMRP.bin`/`201SELCT.bin`/`200FIGHT.bin`/`206GFMCA.bin` Illumination Sword contract - replacement acquisition, tier-5 resource/HUD redraw, exact inventory text, attack audio edge, native base power `20h` (32), and tier-indexed long-reach geometry/color tables |
...
| proc_equivalence | [test_town_first_dialog_oracle.py](proc_equivalence/test_town_first_dialog_oracle.py) | Release-MASM town dialog execution - exact first Felishika script PC, glyph calls, MCGA page hash, sound mailbox, NPC mutation, and restored framebuffer; native matrix executes all 105 castle/town dialog entries plus dedicated prompt, mutation, repeat, automatic-trigger, and insufficient-state branches |
...
| proc_equivalence | [test_gtmcga_town_scroll.py](proc_equivalence/test_gtmcga_town_scroll.py) | Release `111GTMCA.bin` four-way scroll services - exact full-VGA hashes for the side-0 and side-1 dispatch pairs |
| proc_equivalence | [test_ckpd_mcga_background_oracle.py](proc_equivalence/test_ckpd_mcga_background_oracle.py) | Release `209CKPD.bin` mode-4 procedure trace plus direct non-MCGA helper register/framebuffer oracles |
...
| proc_equivalence | [test_fight_selector_domain_oracle.py](proc_equivalence/test_fight_selector_domain_oracle.py) | Complete release-MASM fight selector domain - all 31 area resources (`00h..1Eh`) and all seven sword selector mappings (`00h..06h`) are present, byte-identical in Web assets, and explicitly supported by the VM; native coverage rejects corrupt selectors without mutating host state |
| proc_equivalence | [test_endmo_runtime_oracle.py](proc_equivalence/test_endmo_runtime_oracle.py) | Complete release `250ENDMO` contract - all twelve live procedures, sixteen narration/credits/audio resources, seven credit handlers, script text, entry, and permanent final-screen loop are pinned to the exact MASM binary and the in-place Web VM overlay handoff |
...
| proc_equivalence | [test_spell_espada_oracle.py](proc_equivalence/test_spell_espada_oracle.py) | Sage Yasmin award, learned-only `201SELCT.bin` selection, one-charge `200FIGHT.bin` dispatch, and Espada's directional five-step collision/lifetime contract |
| proc_equivalence | [test_spell_saeta_oracle.py](proc_equivalence/test_spell_saeta_oracle.py) | Sage Hajjar award, learned-only selection, one-charge dispatch, Saeta's directional ten-step projectile, and selected-spell object/wall marker contract |
| proc_equivalence | [test_spell_fuego_oracle.py](proc_equivalence/test_spell_fuego_oracle.py) | Sage Chiriga award, learned-only selection, one-charge dispatch, Fuego's twelve-step ground-following fire, and selected-spell damage marker contract |
| proc_equivalence | [test_spell_lanzar_oracle.py](proc_equivalence/test_spell_lanzar_oracle.py) | Sage Hisham award, learned-only selection, one-charge dispatch, Lanzar's ID-4 flame graphics/damage marker and directional ten-step projectile contract |
| proc_equivalence | [test_spell_rascar_oracle.py](proc_equivalence/test_spell_rascar_oracle.py) | Sage-taught ID-5 Rascar selection, one-charge dispatch, four staggered falling-rock slots, twelve-frame fall, and selected-spell damage marker contract |
| proc_equivalence | [test_spell_agua_oracle.py](proc_equivalence/test_spell_agua_oracle.py) | Sage Saied's ID-6 Agua teaching, learned-only selection, one-charge dispatch, three vertically offset water projectiles, ten-frame directional travel, and damage marker contract |
| proc_equivalence | [test_spell_guerra_oracle.py](proc_equivalence/test_spell_guerra_oracle.py) | Sage Indihar's ID-7 Guerra teaching, learned-only selection, one-charge dispatch, immediate 19-by-36 screen lightning scan, effect teardown, and lightning sound cue contract |
| proc_equivalence | [test_item_kenko_oracle.py](proc_equivalence/test_item_kenko_oracle.py) | Witchcraft Ken'ko Potion shop/name and item-ID contract, pre-dispatch consumption, native +80 HP effect, maximum-life cap, and full-life consumption behavior |
| proc_equivalence | [test_item_juuen_oracle.py](proc_equivalence/test_item_juuen_oracle.py) | Witchcraft Juu-en Fruit shop/name and item-ID contract, pre-dispatch consumption, direct current-to-maximum life assignment, and native inventory sound behavior |
| proc_equivalence | [test_item_elixir_oracle.py](proc_equivalence/test_item_elixir_oracle.py) | Witchcraft Elixir of Kashi shop/name and item-ID contract, selected-spell requirement, and direct restoration of that spell's current charges to its per-spell maximum |
| proc_equivalence | [test_item_chikara_oracle.py](proc_equivalence/test_item_chikara_oracle.py) | Witchcraft Chikara Powder shop/name and item-ID contract plus the native unfiltered seven-byte copy from maximum to current spell-charge slots |
| proc_equivalence | [test_item_holy_water_oracle.py](proc_equivalence/test_item_holy_water_oracle.py) | Holy Water of Acero shop/name and item-ID contract, six shield-tier repair amounts, maximum-strength cap, pre-dispatch consumption, and no-shield behavior |
| proc_equivalence | [test_item_kioku_oracle.py](proc_equivalence/test_item_kioku_oracle.py) | Kioku Feather shop/name and item-ID contract, distinct cue, 120-tick scene-8 fade, result-8 fight handoff, and current-area reload from last-Sage byte C5h |
| proc_equivalence | [test_hero_crest_oracle.py](proc_equivalence/test_hero_crest_oracle.py) | Release Hero's Crest chain: MP30 object 40 at 166/54, persistent byte 12h/mask 08h, 200FIGHT acquisition handler/message, 201SELCT ability slot 9Ch, and the BSMP sentry gate plus all prompt/denial/acceptance text |
| proc_equivalence | [test_elf_crest_oracle.py](proc_equivalence/test_elf_crest_oracle.py) | Release Elf Crest chain: Paguro hut descriptor and victory/reward, LLMP defeat and one-time award mutations, 106TOWN opcode 83h writes to byte 34h/mask 80h and ability byte 9Ah, 201SELCT's first ability slot, and all pre/post-Crest Llama dialog branches |
| proc_equivalence | [test_gmmcga_collected_tears_oracle.py](proc_equivalence/test_gmmcga_collected_tears_oracle.py) | Release `GAME:A3A6`/`GMMCGA:2A1C` Tears-of-Esmesanti HUD contract for every count 0 through 9, including the native non-linear small-Tear slot order and distinct ninth/final Tear sprite |
...
| proc_equivalence | [test_sword_enchantment_oracle.py](proc_equivalence/test_sword_enchantment_oracle.py) | Release `200FIGHT`/`201SELCT`/`206GFMCA` Enchantment Sword oracle - locks the one-time Arrugia reward, tier-6 name/HUD resources, attack/audio latch, `0x7F` base table value with saturation, and longest-reach renderer |
| proc_equivalence | [test_shield_stone_oracle.py](proc_equivalence/test_shield_stone_oracle.py) | Release `212ARMRP`/`201SELCT`/`200FIGHT` Stone Shield oracle - locks tier 3, 180 strength, exact town stock, purchase/equip initialization, tiered damage and break behavior, persisted fields, and 100-point capped Holy Water repair |
| proc_equivalence | [test_shield_clay_oracle.py](proc_equivalence/test_shield_clay_oracle.py) | Release `212ARMRP`/`201SELCT`/`200FIGHT` Clay Shield oracle - locks tier 1, 30 strength, exact town stock, purchase/equip initialization, tiered damage and break behavior, persisted fields, and 80-point capped Holy Water repair |
| proc_equivalence | [test_shield_wise_man_oracle.py](proc_equivalence/test_shield_wise_man_oracle.py) | Release `212ARMRP`/`201SELCT`/`200FIGHT` Wise Man's Shield oracle - locks tier 2, 80 strength, exact town stock, purchase/equip initialization, tiered damage and break behavior, persisted fields, and 90-point capped Holy Water repair |
| proc_equivalence | [test_shields_advanced_oracle.py](proc_equivalence/test_shields_advanced_oracle.py) | Release `212ARMRP`/`201SELCT`/`200FIGHT` advanced shield oracle - locks Honor/Light/Titanium tiers 4/5/6, 300/300/600 maxima, distinct stock, Light's extra reduction shift, damage/break, persistence, and 110/115/120-point capped repair |
| proc_equivalence | [test_progression_lion_key_oracle.py](proc_equivalence/test_progression_lion_key_oracle.py) | Release `MP80`/`200FIGHT`/`201SELCT` Lion Head's Key oracle - locks the Absor hidden-slab reward, distinct save/inventory byte `0099h`, one-time Tesoro unlock, persistent open-door bit, free Arrugia return, Pureza dialog, and downstream rewards |
| proc_equivalence | [test_progression_regular_keys_oracle.py](proc_equivalence/test_progression_regular_keys_oracle.py) | Release normal-key chain: all six persistent cavern pickups, byte 98h count and inventory display, all 22 regular locked doors, zero-key rejection, one-key consumption/cue, persistent door-mask write, and separation from Lion Head's Key doors |
...
| proc_equivalence | [test_select_feruza_oracle.py](proc_equivalence/test_select_feruza_oracle.py) | Release Arrugia persistent function-5 reward, `201SELCT.bin` wearable ownership/equip flow, and `200FIGHT.bin` ID-1 doubled jump-limit gate |
...
The following evidence files are indexed here explicitly and checked by CI:

- [test_fight_game_process_loop_2.py](proc_equivalence/test_fight_game_process_loop_2.py)
- [test_fight_remaining_caverns_resources_oracle.py](proc_equivalence/test_fight_remaining_caverns_resources_oracle.py)
- [test_fight_try_place_3cell_entity_row.py](proc_equivalence/test_fight_try_place_3cell_entity_row.py)
- [test_fight_vga_operations.py](proc_equivalence/test_fight_vga_operations.py)
- [test_gfmcga_roka_background_oracle.py](proc_equivalence/test_gfmcga_roka_background_oracle.py)
- [test_gfmcga_transition_hero_oracle.py](proc_equivalence/test_gfmcga_transition_hero_oracle.py)
- [test_gmmcga_building_fade_oracle.py](proc_equivalence/test_gmmcga_building_fade_oracle.py)
- [test_gmmcga_equipped_spell_oracle.py](proc_equivalence/test_gmmcga_equipped_spell_oracle.py)
- [test_gmmcga_gfx_init_oracle.py](proc_equivalence/test_gmmcga_gfx_init_oracle.py)
- [test_gmmcga_jashiin_speech_clear_oracle.py](proc_equivalence/test_gmmcga_jashiin_speech_clear_oracle.py)
- [test_gmmcga_narration_stream_oracle.py](proc_equivalence/test_gmmcga_narration_stream_oracle.py)
- [test_gmmcga_render_text_char_alt_oracle.py](proc_equivalence/test_gmmcga_render_text_char_alt_oracle.py)
- [test_gmmcga_town_clear_playfield_oracle.py](proc_equivalence/test_gmmcga_town_clear_playfield_oracle.py)
- [test_gmmcga_town_first_frame_hud_oracle.py](proc_equivalence/test_gmmcga_town_first_frame_hud_oracle.py)
- [test_gmmcga_town_hud_label_oracle.py](proc_equivalence/test_gmmcga_town_hud_label_oracle.py)
- [test_gmmcga_town_life_hud_oracle.py](proc_equivalence/test_gmmcga_town_life_hud_oracle.py)
- [test_gmmcga_town_numeric_hud_oracle.py](proc_equivalence/test_gmmcga_town_numeric_hud_oracle.py)
- [test_gmmcga_town_status_lines_oracle.py](proc_equivalence/test_gmmcga_town_status_lines_oracle.py)
- [test_gmmcga_town_text_record_oracle.py](proc_equivalence/test_gmmcga_town_text_record_oracle.py)
- [test_gtmcga_actor_blitters_oracle.py](proc_equivalence/test_gtmcga_actor_blitters_oracle.py)
- [test_gtmcga_capture_playfield_oracle.py](proc_equivalence/test_gtmcga_capture_playfield_oracle.py)
- [test_gtmcga_encode_tile_block_oracle.py](proc_equivalence/test_gtmcga_encode_tile_block_oracle.py)
- [test_mcga_amulet_scanline_oracle.py](proc_equivalence/test_mcga_amulet_scanline_oracle.py)
- [test_mcga_credits_final_stream_oracle.py](proc_equivalence/test_mcga_credits_final_stream_oracle.py)
- [test_mcga_credits_scanline_oracle.py](proc_equivalence/test_mcga_credits_scanline_oracle.py)
- [test_mcga_disp_drv_seg_3_oracle.py](proc_equivalence/test_mcga_disp_drv_seg_3_oracle.py)
- [test_mcga_disp_game_cpu_bound_oracle.py](proc_equivalence/test_mcga_disp_game_cpu_bound_oracle.py)
- [test_mcga_disp_load_setup_oracle.py](proc_equivalence/test_mcga_disp_load_setup_oracle.py)
- [test_mcga_disp_load_setup_rect_oracle.py](proc_equivalence/test_mcga_disp_load_setup_rect_oracle.py)
- [test_mcga_disp_render_a_full_oracle.py](proc_equivalence/test_mcga_disp_render_a_full_oracle.py)
- [test_mcga_disp_render_a_rev_oracle.py](proc_equivalence/test_mcga_disp_render_a_rev_oracle.py)
- [test_mcga_disp_render_ab_ab40_oracle.py](proc_equivalence/test_mcga_disp_render_ab_ab40_oracle.py)
- [test_mcga_disp_render_ab_gseg_oracle.py](proc_equivalence/test_mcga_disp_render_ab_gseg_oracle.py)
- [test_mcga_disp_script_area_oracle.py](proc_equivalence/test_mcga_disp_script_area_oracle.py)
- [test_mcga_disp_tile_render_oracle.py](proc_equivalence/test_mcga_disp_tile_render_oracle.py)
- [test_mcga_disp_tilemap_render_oracle.py](proc_equivalence/test_mcga_disp_tilemap_render_oracle.py)
- [test_mcga_final_scanline_alt_oracle.py](proc_equivalence/test_mcga_final_scanline_alt_oracle.py)
- [test_mcga_gfx_update_da_oracle.py](proc_equivalence/test_mcga_gfx_update_da_oracle.py)
- [test_mcga_palette_dispatch_oracle.py](proc_equivalence/test_mcga_palette_dispatch_oracle.py)
- [test_mcga_render_entries_oracle.py](proc_equivalence/test_mcga_render_entries_oracle.py)
- [test_mcga_sprite_completion_palette_oracle.py](proc_equivalence/test_mcga_sprite_completion_palette_oracle.py)
- [test_mcga_sprite_frame_prepare_oracle.py](proc_equivalence/test_mcga_sprite_frame_prepare_oracle.py)
- [test_mcga_sprite_object_init_oracle.py](proc_equivalence/test_mcga_sprite_object_init_oracle.py)
- [test_mcga_sprite_palette_timing_oracle.py](proc_equivalence/test_mcga_sprite_palette_timing_oracle.py)
- [test_mcga_sprite_palette_write_oracle.py](proc_equivalence/test_mcga_sprite_palette_write_oracle.py)
- [test_mcga_title_sweep_assets_oracle.py](proc_equivalence/test_mcga_title_sweep_assets_oracle.py)
...
- [test_opdemo_palette_lookup_oracle.py](proc_equivalence/test_opdemo_palette_lookup_oracle.py)
- [test_stick_f7_restore_oracle.py](proc_equivalence/test_stick_f7_restore_oracle.py)
- [test_stick_overlay_swap_oracle.py](proc_equivalence/test_stick_overlay_swap_oracle.py)
- [test_stick_timer_int60_oracle.py](proc_equivalence/test_stick_timer_int60_oracle.py)
- [test_town_castle_music_oracle.py](proc_equivalence/test_town_castle_music_oracle.py)
- [test_town_walk_right_col.py](proc_equivalence/test_town_walk_right_col.py)
- [test_ympd_mcga_background_oracle.py](proc_equivalence/test_ympd_mcga_background_oracle.py)
- [test_compute_scroll_offset_b.py](regression/test_compute_scroll_offset_b.py)
- [test_player_record_contract.py](regression/test_player_record_contract.py)
