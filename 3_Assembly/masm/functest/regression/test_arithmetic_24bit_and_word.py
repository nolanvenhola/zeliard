...
def main() -> int:
    ...
    failures = []
    hero_hp_subtract = fight_base + resolve_proc('fight', 'subtract_from_player_HP')
    hero_almas_add = fight_base + resolve_proc('fight', 'hero_almas_add')
    ...
    target = find_pattern_target(town_path, bytes.fromhex('0106860010168500c3'), town_base)
    SCENARIOS = [
    ...
    target = find_pattern_target(town_path, bytes.fromhex('8a1e85002ada7301c3'), town_base)
