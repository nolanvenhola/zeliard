...
def probe(h, proc, si_byte, dl):
    h.write_byte(0x1000, si_byte)
    snap = h.snapshot()
    r = h.call_function(proc, regs={'si': 0x1000, 'dx': dl},
                        max_steps=50)
...
def main() -> int:
    ...
    proc = base + resolve_proc('fight', 'match_dl_within_3',
                               fallback_names=('game_func_82',))
    h = TasmHarness(flat, base)
    stub_video_drivers(h)
    print(f'match_dl_within_3 @ CPU 0x{proc:04X}')
