#!/usr/bin/env python3
"""Release-MASM oracle for GMMCGA's equipped-spell renderer at 25E2h."""
...
ENTRY, RET_SENTINEL = 0x25E2, 0x0080
EXPECTED_RECTS = (
    0x4B78EDCB41024AE4, 0x38EC34BD15886153, 0x27BE1BB89569FF7D,
    0x084168335E2CB4F8, 0x22C4134884DF7F89, 0xAE4412E5B02DFFDB,
    0xCB99E1ED8440BF12,
)
...
def render_spell(spell: int) -> tuple[int, bool]:
    ...
    mu.reg_write(UC_X86_REG_AX, spell)
    mu.reg_write(UC_X86_REG_BX, 0x37A4)
    ...
    rect = b"".join(frame[(164 + row) * 320 + 222:
                          (164 + row) * 320 + 238] for row in range(16))
    return fnv1a64(rect), mu.reg_read(UC_X86_REG_IP) == RET_SENTINEL


def main() -> int:
    results = [render_spell(spell) for spell in range(1, 8)]
    ok = all(returned and rect == EXPECTED_RECTS[index]
             for index, (rect, returned) in enumerate(results))
    print("gmmcga_equipped_spell: " + ("PASS" if ok else "FAIL") +
          " rects=" + ",".join(f"{rect:016x}"
                                 for rect, _returned in results))
    print("VERDICT: " + ("PASS" if ok else "FAIL") +
          ": release-MASM GMMCGA equipped-spell renderer")
