#!/usr/bin/env python3
"""MASM oracle for 106TOWN shop selection animation loop."""
...
def check_case(name: str, start_al: int, count: int, dialog_pos: int,
               xlat_base: int) -> list[str]:
    ...
    h.write_data(OFF_GVAR_SEL_XLAT, bytes((xlat_base + i) & 0xFF
                                           for i in range(0x40)))

    result = h.call_function(
        SHOP_SEL_ANIM_ENTRY,
        regs={"ax": (0x7700 | start_al), "cx": count, "si": 0x1234, "di": 0x5678},
    ...
    for idx, regs in enumerate(init_regs[:count]):
        expected_al = (xlat_base + start_al + idx) & 0xFF
...
def main() -> int:
    ...
    failures += check_case("three_items", start_al=2, count=3,
                           dialog_pos=0x2000, xlat_base=0x80)
    failures += check_case("one_item", start_al=5, count=1,
                           dialog_pos=0x3450, xlat_base=0x90)

    if failures:
        print("town shop-selection-animation oracle mismatches:")
        for failure in failures:
            print(f"  - {failure}")
        print("VERDICT: FAIL")
        return 1

    print("town shop-selection-animation oracle: init/draw call sequence matches")
    print("VERDICT: PASS")
    return 0
