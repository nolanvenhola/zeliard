...
CHECK_GOLD_PATTERN = bytes.fromhex('8a1e85002ada7301c3')
...
def call_check(gold_hi, gold_lo, gold_mid, request_hi, request_low_word):
    ...
    target = find_pattern_target(TOWN_FLAT, CHECK_GOLD_PATTERN, LOAD_BASE)
...
def main():
    ...
    target = find_pattern_target(TOWN_FLAT, CHECK_GOLD_PATTERN, LOAD_BASE)
    print(f'town.bin gold comparator pattern -> function at 0x{target:X}')
    ...
    if n_pass == n_total:
        ...
        print('\nVERDICT: PASS: town slot 0x600A compares the 24-bit hero gold layout without writes.')
        return 0
    else:
        print('  At least one probe failed — see above.  Layout may need')
        print('  further refinement.')
        print(f'\nVERDICT: REFUTED: only {n_pass}/{n_total} gold comparator probes passed.')
        return 1
