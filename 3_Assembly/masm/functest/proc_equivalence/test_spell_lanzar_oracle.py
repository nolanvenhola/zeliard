#!/usr/bin/env python3
"""Release-byte oracle for Lanzar teaching, selection, and flame projectile."""
...
def main() -> int:
    ...
    # IDs 1..4 all use initializer 884Dh. The secondary table maps Lanzar
    # ID 4 back to 8AF7h, the directional ten-step collision handler.
    ...
    checks = {
        "hisham_dialog": b"Spell of Flame: Lanzar." in kenjp,
        "lanzar_award_id4": kenjp.count(award) == 1,
        "lanzar_name": b"Lanzar\x00" in select,
        ...
        "lanzar_initializer_884d": fight.count(dispatch_table) == 1,
        "lanzar_secondary_8af7": fight.count(secondary_table) == 1,
        "lanzar_ten_step_projectile": fight.count(lanzar_step) == 1,
        "selected_spell_damage_marker": fight.count(object_marker) == 1,
    }
    for name, passed in checks.items():
        print(f"spell_lanzar:{name}: {'PASS' if passed else 'FAIL'}")
    ok = all(checks.values())
    print("VERDICT: " + ("PASS" if ok else "FAIL") +
          ": release MASM defines Lanzar teaching, charge, flame travel, and damage marker")
