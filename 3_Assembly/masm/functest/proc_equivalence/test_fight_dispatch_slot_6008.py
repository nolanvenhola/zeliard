...
Probes the function whose address sits at fight.bin's current dispatch slot 0x600C
...
SLOT_ADDR  = 0x600C          # dispatch slot we're probing
...
def main():
    ...
    #            return early.
    # ------------------------------------------------------------------
    CALL_TARGET = 0x92B8   # check_collision_E2, biased for rebuilt-bin header
    ...
    if early_return_when_high and passable_mutates and blocked_no_mutation:
        ...
        print('\nVERDICT: PASS: dispatch slot 0x600C is a bounded movement primitive gated by collision CF.')
        return 0
    else:
        print('\n  --> Function does NOT match the expected bounded-mutator shape.')
        print('VERDICT: REFUTED: dispatch slot 0x600C did not match the bounded movement primitive shape.')
        return 1
