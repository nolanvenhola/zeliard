#!/usr/bin/env python3
"""Release-byte oracle for Rascar teaching, selection, and falling rocks."""
...
def main() -> int:
    ...
    four_rock_initializer = bytes.fromhex(
        "b9 04 00 51 b0 06 f6 e1 05 02 00 03 06 80 00 "
        "3b 06 02 c0 72 04 2b 06 02 c0 89 04 2e ff 16 "
        "1a 01 24 03 8a 26 82 00 80 ec 03 2a e0 80 e4 3f"
    )
    four_rock_step = bytes.fromhex(
        "fe 44 04 80 7c 04 0c 73 30 b9 04 00 51 80 44 02 "
        "02 80 64 02 3f e8 7b 00 83 c6 10 59 e2 ee c3"
    )
    object_marker = bytes.fromhex(
        "8a 47 05 0c 40 24 e0 8a 26 9d 00 fe c4 0a c4 88 47 05"
    )
    checks = {
        "sage_dialog": b"Falling Rocks: Rascar." in kenjp,
        "rascar_award_id5": kenjp.count(award) == 1,
        "rascar_name": b"Rascar\x00" in select,
        ...
        "rascar_primary_88a8": fight.count(dispatch_table) == 1,
        "rascar_secondary_8b64": fight.count(secondary_table) == 1,
        "four_staggered_rocks": fight.count(four_rock_initializer) == 1,
        "four_rocks_fall_two_rows": fight.count(four_rock_step) == 1,
        "selected_spell_damage_marker": fight.count(object_marker) == 1,
    }
    for name, passed in checks.items():
        print(f"spell_rascar:{name}: {'PASS' if passed else 'FAIL'}")
    ok = all(checks.values())
    print("VERDICT: " + ("PASS" if ok else "FAIL") +
          ": release MASM defines Rascar teaching, charge, four-rock spawn, fall, and damage")
