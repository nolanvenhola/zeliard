...
def probe(h, proc, slot_byte_value, dl):
...
def main() -> int:
    ...
    proc = load_base + resolve_proc('fight', 'entity_slot_write_tagged',
                                    fallback_names=('game_func_89',))
    ...
    print(f'entity_slot_write_tagged @ CPU 0x{proc:04X}')
    print()

    # Probe A: bit-7 clear -> direct write
    slot, ext, _ = probe(h, proc, slot_byte_value=0x05, dl=0x42)
    ...
    slot, ext, _ = probe(h, proc, slot_byte_value=0x85, dl=0x42)
    ...
    slot, ext, _ = probe(h, proc, slot_byte_value=0xFF, dl=0x99)
