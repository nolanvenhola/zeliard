#!/usr/bin/env python3
"""Release-MASM oracle for the Spirit Sword equipment tier."""
...
def main() -> int:
    ...
    checks = {
        "armory_name": armory.count(b"Spirit sword\0") == 1,
        "armory_replaces_equipped_tier": armory.count(weapon_commit) == 1,
        "inventory_hud_name": select.count(b"Spirit\0    Sword\0") == 1,
        "attack_state_and_audio_edge": fight.count(attack_latch) == 1,
        "tier_3_base_power": fight.count(power_path) == 1 and
            power_path[-6:][2] == 4,
    }
    for name, passed in checks.items():
        print(f"spirit_sword:{name}: {'PASS' if passed else 'FAIL'}")
    ok = all(checks.values())
    print("VERDICT: " + ("PASS" if ok else "FAIL") +
          ": release MASM defines Spirit Sword as equipped tier 3, power 4")
